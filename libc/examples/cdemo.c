...
int main(int argc, char **argv) {
    ...
    /* `argv[argc]` обязан быть нулевым указателем — так говорит стандарт
     * языка (C11, 5.1.2.2.1), и чужие программы на это рассчитывают: они
     * идут по массиву, пока не встретят ноль, и `argc` не смотрят вовсе.
     * Проверка стоит здесь потому, что цена её нарушения уже заплачена:
     * массив без завершителя убил Lua в разборе её собственных
     * аргументов, а все наши программы читают `argc` и ничего не
     * замечали. */
    if (argv[argc] == NULL) {
        ok("argv ends with a null pointer");
    } else {
        fail("argv ends with a null pointer");
    }
