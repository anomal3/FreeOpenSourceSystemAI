...
impl DiskBus {
    /// Сектор у этой шины — 4096 байт, а не 512.
    ///
    /// Спрашивает расписание стенда: установка на такой диск пересоздаёт
    /// **общий** диск цепочки с другой геометрией, и всякий сценарий
    /// установленной системы, запущенный после, увидит разметку, которой его
    /// QEMU не понимает, — прошивка уйдёт в свою оболочку.
    #[must_use]
    pub const fn sector_is_4k(self) -> bool {
        matches!(self, DiskBus::Nvme4k)
    }
}
...
pub fn command(opts: &RunOptions, built: &Built) -> Result<Command> {
    ...
    // Имя нужно сценарию, которому надо выдернуть эту клавиатуру: пока она
    // подключена, `sendkey` QEMU отдаёт нажатия ей, а не PS/2 (проверено
    // счётчиком отчётов), и до контроллера PS/2 не доходит ничего.
    cmd.args(["-device", &format!("usb-kbd,bus={},id=usbkbd", opts.usb.bus())]);
