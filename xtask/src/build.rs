...
pub fn build_all(opts: &BuildOptions) -> Result<Built> {
    ...
    // Программы на C собираются тем же условием и по той же причине, но
    // отдельным вызовом: у них свой компилятор, своя libc и своё место сборки.
    // Отсутствие набора — не повод останавливать сборку образа целиком: на
    // машине без LLVM система обязана собираться и работать, только без этих
    // программ. Поэтому отказ здесь становится строкой, а не ошибкой.
    let mut programs = programs;
    if opts.kernel || opts.initrd {
        match crate::cbuild::build_c_programs(opts.arch) {
            Ok(built) => programs.extend(built),
            Err(err) => say!("программы на C пропущены: {err}"),
        }
    }
    ...
    let initrd = if opts.initrd {
        ...
        //
        // Одного ключа мало, и это выяснилось на живой машине: `sshd` пускает
        // только того, кто есть в `/etc/passwd`, а запись с `uid 0` отвергает
        // нарочно — root по сети не входит никогда. У системы, загруженной с
        // носителя, своего `/etc/passwd` нет, поэтому здесь кладётся и он.
        // Ключ в `root/.ssh` не кладётся вовсе: там он не сработает никогда.
        //
        // Следствие, о котором надо знать: ядро читает этот файл безусловно, и
        // сеанс живой системы перестаёт быть root — он идёт от имени `roman`
        // (uid 1000). Отнимает это ровно две возможности: обновление системы и
        // снятие чужой программы. Первая носителю недоступна и так (слотов у
        // флешки нет), вторая для отладки не нужна.
        if let Some(path) = std::env::var_os("FREEOS_AUTHORIZE") {
            ...
            say!("          входить в такую систему: ssh roman@<адрес>, не root@");
            // Формат — тот же, что пишет установщик (см. `installer::account`).
            // Поля пароля заполнены заведомо негодно, и это не небрежность:
            // входа по паролю в системе нет вовсе, а нули на месте отпечатка
            // видно глазом — «сюда паролем не входят».
            generated.push((
                String::from("etc/passwd"),
                b"# FreeOS accounts, put here by FREEOS_AUTHORIZE: a debug image, never a release\n\
                  # name:uid:gid:mode:home:algorithm:salt:digest\n\
                  roman:1000:1000:0750:/home/roman:fnv1a64-4096:0000000000000000:0000000000000000\n"
                    .to_vec(),
            ));
...
pub const USER_PROGRAMS: [&str; 35] = [
    ...
    "taskmgr", "devmgr", "dotnet", "sftp-server",
];
...
pub fn check(arches: &[Arch]) -> Result<()> {
    ...
    // Формат пакета, запись о слотах и вход по ключу — то же самое рассуждение,
    // что у HID. Откат по счётчику попыток, разбор порванной записи и раскладка
    // подписанного блока SSH проверяются здесь за секунды; в эмуляторе то же
    // утверждение стоило бы загрузок и получаса, а ошибка в раскладке подписи
    // выглядела бы у клиента неотличимо от чужого ключа.
    // Дерево устройств — то же рассуждение, но с новым поворотом: этот разбор
    // работает **до** того, как ядру станет известно, сколько в машине памяти,
    // и ошибается он смещениями внутри чужого блоба. На телефоне цена такой
    ...
    // `xtask` попал в этот список с фазы 45, и по той же причине, что и
    // `user-abi`: договор теперь выписан **дважды** — для стороны Rust и для
    // стороны C (`libc/freeos/freeos-syscall.h`). Тесты здесь сверяют вторую
    // запись с первой и заодно собирают образцовую программу на C компилятором
    // хоста, проверяя, что она обычная, а не написанная под наши особенности.
    // `sysconf` здесь по той же причине, что и остальные, но повод у него свой и
    // его стоит назвать: разбор `/etc` жил в ядре, а ядро под хост не
    // собирается — значит `#[cfg(test)]` в нём не запускался никогда. Ошибка в
    // разборе часового пояса выглядит как «система забыла настройку» и требует
    // загрузки, чтобы её увидеть; здесь она стоит миллисекунды.
    // `btrfs` стоит первым по важности, а не по алфавиту случайно: он читает
