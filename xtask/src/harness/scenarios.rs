...
pub const ALL: &[Scenario] = &[
    Scenario {
        ...
        steps: &[
            ...
            // Срок тот же, что до баннера, и это не щедрость: между баннером и
            // оболочкой лежит вся загрузка — перечисление initrd, диски, USB,
            // композитор. Прежние шестьдесят секунд хватало ровно до тех пор,
            // пока хост был свободен: 18.09 на занятой машине отладочная
            // загрузка aarch64 не уложилась в них дважды подряд, а с запасом
            // проходит. Срок шага отвечает на вопрос «не повисло ли», а не
            ...
            // Список задач — в журнал каждой загрузки, и не для красоты.
            // Заблокированная задача теперь называет, **чего** она ждёт, а не
            // только что она не претендует на процессор. Ровно этой строки не
            // хватило открытому дефекту «init на aarch64 иногда замолкает»:
            // по журналу видно, что задача есть и молчит, и ничем не отличить
            // «встала на замке» от «её обошли».
            Step::Line("tasks"),
            // Супервизор назван по имени, а не словом «program». Проверяется
            // это раньше повода ожидания нарочно: его строка в таблице стоит
            // выше, а шаг ждёт то, что **ещё не проехало**.
            Step::Await("#1 init", 15_000),
            // Спящая задача есть на всякой живой машине: оболочка ждёт ввода,
            // службы спят до срока. Если ни одна не назвала повода — значит
            // диагностика молчит, а это ровно то, что чинится.
            Step::Await("waiting for", 15_000),
            // Доля простоя меряется временем, а не тиками, и окном с запуска
            // планировщика, а не со включения таймера. Проверяется здесь само
            // наличие честного знаменателя: прежняя строка считала тики за всю
            // загрузку и на отладочной сборке занижала долю втрое.
            Step::Await("since scheduling started", 15_000),
    ...
    Scenario {
        ...
        steps: &[
            ...
            // Отчёты приходят прерыванием, а не подбираются сроком ожидания.
            // Разница снаружи не видна никак — ввод работает и так и так, — и
    ...
    Scenario {
        ...
        steps: &[
            ...
            // Отчёты приходят прерыванием, а не подбираются сроком ожидания.
            // Снаружи разницы нет никакой — ввод работает и так и так, — и
            // показывает её только число.
            //
            // Сигнал берётся щелчком мыши, а не набором: отчёт мыши идёт через
            // тот же периодический список, а спрашивается счётчик по серийной
            // линии — то есть проверка не зависит от того, дошла ли до
            // строкового редактора каждая клавиша.
            Step::Click,
            Step::Line("irq ehci"),
            Step::Await("ehci woke the system", 15_000),
    ...
    Scenario {
        ...
        steps: &[
            ...
            // Главная проверка этой фазы: по линии, названной прошивкой,
            // действительно приходят сигналы. Без неё сценарий остался бы
            // зелёным и у карты, которая объявила прерывания, а будится по
            // сроку — то есть опросом под другим именем. Команда идёт **до**
            // выхода из оболочки: после него набирать её уже некому.
            Step::Line("irq net"),
    ...
    Scenario {
        ...
        steps: &[
            ...
            Step::Key("alt-shift"),
            Step::Await("keyboard    : layout EN (us)", 15_000),
            // Замер задержки (см. `kernel::latency`). Здесь, а не в начале: к
            // этому месту стол перерисован десятки раз, и через драйвер прошли
            // все нажатия сценария. Им нашли дефект «задача USB готова, но ждёт
            // процессора 400+ мс» (25.09.2026); проверяется только то, что отчёт
            // есть, — числа читает человек, порога у них пока нет.
            Step::Line("tasks"),
            Step::Await("latency    : interrupts held off", 15_000),
    ...
    Scenario {
        ...
        steps: &[
            ...
            // Дождаться КОНЦА первого списка, а не строки в его середине.
            // Иначе стенд набирает следующую команду, пока программа ещё
            // печатает, и та попадает не в редактор оболочки, а в остаток
            // терминала. Оболочка такой хвост забирает (`replay_pending`), но
            // успевает не всегда, и на занятом хосте сценарий падал трижды
            // подряд, а на свободном проходил. Проверять здесь надо `ls`, а
            // не скорость машины.
            Step::Await("ls: ", 30_000),
            ...
            // Фаза 55a: владение процессом считается ссылками, и уходящая
            // задача обязана быть последней, кто на него ссылается. Строка
            // печатается, когда это не так, — то есть когда `Drop`
            // пространства не случился вовремя. Снаружи такое выглядит просто
            // как утечка кадров, а здесь называет себя.
    ...
    Scenario {
        ...
        steps: &[
            ...
            // И причина, а не следствие: сколько длилась самая долгая печать
            // с запрещёнными прерываниями. Раньше здесь стояла проверка
            // «спасение сработало хотя бы раз» — она проверяла, что помогает
            // заплатка. Теперь проверяется, что дефекта нет: до выноса
            // рисования из промежутка молчания было 920 000 мкс на x86-64 и
            // 867 000 на AArch64, стало пятнадцать-двадцать тысяч. Предел
            // поставлен на порядок ниже прежнего числа и на порядок выше
            // нынешнего: он ловит возврат дефекта, а не дрожание занятого
            // хоста, который добавляет к замеру собственные остановки.
            Step::AtMost("  deafest  ", 200_000, 15_000),
    ...
    Scenario {
        ...
        steps: &[
            ...
            // Порядок шагов — тот, в каком печатает программа: `Await` ищет
            // вперёд от места, где остановился прошлый шаг.
            //
            // Фаза 55c: под замком лежит НЕатомарный счётчик. Атомарный
            // сошёлся бы и без замка, и проверка доказывала бы только то, что
            // атомики работают.
            Step::Await("threads: guarded counter is 4000 of 4000", 60_000),
            // И замок обязан был вправду усыплять, а не крутить процессор:
            // крутящий выглядит снаружи так же.
            Step::AtLeast("threads: the lock put someone to sleep ", 1, 15_000),
    ...
    Scenario {
        ...
        steps: &[
            ...
            Step::Line("tasks"),
            // Имя, а не слово «program»: с `1159301` таблица задач называет
            // программу по имени, и прежняя строка проверяла вывод, которого
            // больше нет. Ширину поля задаёт `{:<8}` в `sched`.
            Step::Await("nap      blocked", 15_000),
    ...
    Scenario {
        ...
        steps: &[
            ...
            // ── Электропитание ───────────────────────────────────────────────
            //
            // «Сеть» третья сверху, «Электропитание» — десятое: семь вниз.
            // Проверяется здесь не «экран погас» (это делает сценарий
            // `screen-off` живой загрузкой), а то, ради чего раздел заведён:
            // срок задаётся окном и **записывается на диск**. Система здесь
            // установленная, и `/etc` у неё настоящий.
            Step::Key("left"),
            Step::Repeat("down", 7),
            Step::Key("ret"),
            Step::Wait(2_000),
            Step::Shot("07-power"),
            // Список: 1 минута, 5 минут, 10 минут, 30 минут, «Никогда».
            // Третий пункт — десять минут, и выбран он не наугад: 600 секунд
            // не совпадают ни с умолчанием (300), ни с нулём, поэтому строка
            // журнала доказывает именно выбор, а не то, что и так стояло.
            Step::Repeat("down", 2),
            Step::Key("ret"),
            Step::Await("settings    : screen off after 600 s saved", 15_000),
            // И «Никогда» — последний пункт, ещё два вниз. Он же оставляет
            // машину в состоянии, в котором следующие сценарии цепочки не
            // потемнеют посреди работы: экран у них не гаснет вовсе.
            Step::Repeat("down", 2),
            Step::Key("ret"),
            Step::Await("settings    : screen off never saved", 15_000),

            // «Панель управления» — последний, одиннадцатый раздел: плитки всех
            // остальных, и Enter на первой открывает «Экран и оформление».
            Step::Key("left"),
            Step::Key("down"),
            Step::Wait(3_000),
            Step::Shot("08-overview"),
            Step::Key("ret"),
            Step::Key("ret"),
    ...
    Scenario {
        ...
        steps: &[
            ...
            // Фаза 52: кадры приходят прерыванием, а не опросом. Проверяется не
            // «прерывания настроены», а **как задача собирается ждать**: карта,
            // не принявшая вектор, осталась бы на опросе и сказала бы об этом
            // сама — этой строки тогда не было бы вовсе.
            //
            // Шаг стоит здесь, а не первым: он ждёт строку, которая идёт в
            // журнале **после** адреса карты, а шаг ждёт то, что ещё не
            // проехало.
            Step::Await("network     : receive task ", 30_000),
            Step::Expect("woken by interrupt"),
            // И то же самое со стороны планировщика, а не драйвера: строка
            // ожидания бывает только у задачи, которая **действительно** спит
            // на прерывании. Драйвер мог бы сказать «настроено» и всё равно
            // крутиться на будильнике — здесь это было бы видно.
            Step::Await("freeos> ", 60_000),
            Step::Line("tasks"),
            ...
            // Ещё раз — в конце, когда за спиной вся работа сценария. Первый
            // снимок брался через полсекунды после запуска планировщика, и
            // счётчик пробуждений там ничего не говорит: окно короче, чем
            // разгон. Здесь окно — весь сценарий, и число сравнимо с тем, что
