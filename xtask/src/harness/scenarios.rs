...
pub const ALL: &[Scenario] = &[
    ...
    Scenario {
        ...
        steps: &[
            ...
            // Сколько раз нажать Tab — считается от порядка окон по глубине, и
            // фаза 47b его изменила. Раньше окно состояния заводило ядро, и оно
            // ложилось **под** оболочку; теперь его открывает программа, то
            // есть позже и поверх. Стопка стала «терминал, монитор, файлы»
            // вместо «монитор, терминал, файлы», и обход по кругу приходит к
            // оболочке на шаг раньше.
            Step::Key("tab"),
            ...
            // Закрытие возвращает фокус верхнему из оставшихся окон — и с фазы
            // 47b им оказывается **монитор**, а не оболочка. Причина та же, что
            // и у счёта нажатий Tab выше: окно монитора заводит программа, то
            // есть позже оболочки и поверх неё, а прежнее окно состояния
            // заводило ядро — оно ложилось под оболочку, и фокус возвращался
            // прямо ей.
            //
            // Поэтому лишний Tab: команда ниже набирается с клавиатуры и должна
            // попасть в терминал, а не в чужое окно.
            ...
            Step::Expect("of 29 programs from /bin"),
            Step::Key("f1"),
            Step::Await("desktop     : menu opened", 15_000),
            Step::Key("right"),
            Step::Wait(1_000),
            ...
            //
            // Пункт **пятый**, а не шестой: с фазы 47b монитор системы — это
            // программа из `/bin`, и из списка окон ядра он ушёл. Меню находит
            // его там же, где остальные программы, а предлагать два разных
            // способа открыть одно окно незачем.
            Step::Key("f1"),
            Step::Await("desktop     : menu opened", 15_000),
            Step::Repeat("down", 4),
            Step::Key("ret"),
