...
//!
//! Потому что экран разный: у каждой архитектуры свой режим от загрузчика (на
//! 14.09.2026 — 1280×720 у OVMF и 1024×768 у `ramfb` на `virt`), а с фазы С6a
//! он ещё и меняется на ходу. От размера экрана зависит всё: масштаб шрифта,
...
pub enum Aim {
    ...
    EmptyBelow,
    /// Трей: часы у правого края панели задач. Область попадания часов
    /// тянется до края плашки, и тридцать точек от правого края экрана — это
    /// ещё часы при любой их ширине.
    Tray,
    /// Строка списка в окне «Files» с таким номером сверху.
    ///
    /// Геометрия окна программы: заголовок 44, панель инструментов 48, шапка
    /// столбцов около 27, строка 32 с зазором 2. Прицел — середина строки, и
    /// запас в шестнадцать точек покрывает разницу в высоте шапки между
    /// начертаниями. По ширине — 340 точек от левого края: и с боковой
    /// колонкой (240), и без неё это список.
    FilesRow(i32),
    ...
    /// Точка содержимого окна программы, отсчитанная от его правого нижнего
    /// угла: `(dx, dy)` левее и выше. Так стенд попадает в кнопки окна
    /// сообщения (фаза N7c): размер окна зависит от ширины текста, а место
    /// кнопок от угла — нет.
    ProgramCorner(&'static str, i32, i32),
...
pub fn resolve(aim: Aim, log: &str) -> Result<(i32, i32)> {
    ...
    let point = match aim {
        ...
        Aim::Tray => (width - PANEL_INSET - 30, height - PANEL_INSET - PANEL_H / 2),
        Aim::FilesRow(n) => {
            let rect = window(log, "Files")?;
            (rect.x + 340, rect.y + TITLE_H + 48 + 27 + n * 34 + 16)
        }
        Aim::FilesBody => {
            let rect = window(log, "Files")?;
        ...
        // Высота окна из журнала уже включает полосу заголовка, поэтому низ
        // прямоугольника — низ содержимого.
        Aim::ProgramCorner(title, dx, dy) => {
            let rect = window(log, title)?;
            (rect.x + rect.w - dx, rect.y + rect.h - dy)
        }
