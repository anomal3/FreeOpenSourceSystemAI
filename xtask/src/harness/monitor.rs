...
impl Monitor {
    ...
    /// Сбросить машину, ничего не спрашивая у гостя.
    ///
    /// Ровно то же, что кнопка «Reset» на корпусе: гость не получает ни
    /// уведомления, ни времени дописать. Ответ не проверяется — машина
    /// перезапускается прямо в момент его разбора, и требовать от монитора
    /// пустой строки значило бы падать на успешной команде.
    pub fn system_reset(&mut self) -> Result<()> {
        self.command("system_reset").map(|_| ())
    }

    /// Нажать кнопку питания.
    ///
    /// Для гостя это событие ACPI, а не команда: чипсет выставляет признак,
    /// система разбирает его и гасит себя сама — или не гасит, и тогда машина
    /// продолжает работать. Тем эта проверка и ценна.
