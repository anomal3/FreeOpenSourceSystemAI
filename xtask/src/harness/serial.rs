...
impl SerialLine {
    ...
    /// помещалась в FIFO целиком, а команды покороче проходили. Настоящее
    /// лекарство — не держать прерывания запрещёнными так долго, и оно за
    /// пределами стенда; но и без него **терминал так не печатает**: человек
    /// набирает по символу, и линия без управления потоком рассчитана именно на
    /// это. Стенд был единственным, кто стрелял в неё очередью.
