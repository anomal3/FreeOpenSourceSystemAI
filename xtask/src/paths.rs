...
/// Диск с томом btrfs, который стенд подключает вторым носителем.
///
/// Лежит в каталоге **воркера**, а не в общем `build/`: сценарий `btrfs-read`
/// ни с кем диск не делит и в цепочку установленной системы не входит, а
/// значит два воркера могли бы готовить его одновременно.
pub fn btrfs_disk(arch: Arch, release: bool) -> PathBuf {
