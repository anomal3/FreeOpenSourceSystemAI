...
//! # Про адреса
//!
//! Заголовок несёт адреса, по которым загрузчик разложит части образа. База
//! 0x40078000, ядро — на 0x8000 выше, то есть ровно 0x40080000: под этот адрес
//! `boot-bare` и скомпонован. Расхождение здесь стоило бы молчащей машины —
//! переходы внутри образа PC-относительные и продолжают работать, а всё
//! остальное смотрит мимо.
//!
//! Числа не угаданы: они прочитаны из заводского `recovery.img` (`--read`).
//! Соглашение семейства совпало с ним везде, кроме адреса меток, — там оно
//! давало 0x478F8000 вместо настоящего 0x47880000.
//!
//! # Чем ещё образ обязан походить на заводской
//!
//! Тремя вещами, и каждая выяснилась запуском, потому что молчаливый отказ
//! загрузчика выглядит одинаково в любом случае:
//!
//! * ядро лежит **сжатым** (`--gzip`) — заводское начинается с `1f 8b 08 00`,
//!   и голый образ загрузчику распаковать нечем;
//! * RAM-диск есть и не пуст (`--ramdisk`), хотя нашему образу он не нужен;
//! * строка запуска несёт `bootopt=64S3,32N2,64N2` — по ней LK узнаёт, что
//!   ядро 64-битное. У заводского образа она ровно такая же.
...
/// Адрес, по которому ядро ложится в память и под который оно скомпоновано.
///
/// Он же у QEMU `virt`: тот кладёт образ по началу ОЗУ (`0x40000000`) плюс
/// смещение из заголовка arm64 (`0x80000`). Совпадение не случайное — оба
/// следуют соглашению Linux, — и на нём держится сценарий стенда, где машина
/// описана деревом устройств (см. `harness::scenarios::Target::DeviceTree`).
pub const KERNEL_LOAD: u64 = MTK_BASE + KERNEL_OFFSET;
...
/// отказа, ни сообщения — машина исправно крутится и молчит.
pub fn build_bare(expected_load: u64, probe: bool, keepalive: bool) -> Result<PathBuf> {
    ...
    if probe {
        features.push("mtk-probe");
    }
    if keepalive {
        features.push("mtk-keepalive");
    }
