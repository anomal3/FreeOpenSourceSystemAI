...
| **Devices that wake us** | The network card, the virtio disk and the SATA controller no longer keep the processor busy while they answer: the waiting task sleeps and an interrupt wakes it, with a short deadline behind it so a lost signal costs one millisecond rather than a hung machine. Both kinds of message-signalled interrupt are spoken — MSI-X and, since the SATA controller offers nothing else, plain MSI. Machines that cannot deliver either (a GICv3 with no v2m) fall back to polling and say so out loud. `irq <device>` answers the question a green test cannot: not whether the vector was configured, but whether a single signal ever arrived |
| **Interrupt lines, not guesses** | A device with no MSI at all -- the USB 2.0 and 1.1 controllers, the e1000 card -- still has an interrupt line, and only the firmware knows which one. The kernel reads it out of the `_PRT` table in AML: not by running the bytecode (that way lies ACPICA) but by reading it, the same way it already digs the power-off object out. The line is shared and level-triggered, so every driver on it is called and each one has to check its own status register. `pci` prints the line each device landed on |
| **A print no longer deafens the machine** | Drawing the text console used to happen inside the print itself, with interrupts off: one line could hold the machine silent for nine tenths of a second, long enough to lose everything typed into it and to stop the clock. The console now keeps two buffers -- the text it wants and the text the screen shows -- and printing only touches the first. The difference is carried over afterwards, one cell at a time, with the machine fully alive in between. Scrolling became a memory copy. `input` prints the longest silence, and a scenario holds it to a bound |
...
| **v0.8** | Real hardware: a phone — no UEFI, no ACPI, started from the `recovery` partition, USB in device mode, **a working touchscreen** ([docs/PHONE.md](docs/PHONE.md)) | the phone works: the kernel draws on the screen, talks to a computer over the cable, and windows are moved and closed with a finger; the device-tree boot is checked on the bench too — QEMU `virt` starts the kernel with no firmware and no ACPI; memory, the serial line, the interrupt controller and the PCIe bus come from the tree, the kernel assigns the BARs itself, and a virtio disk and a USB keyboard work behind them; bridges behind it get their bus numbers and windows from the kernel, and INTx lines come from `interrupt-map`; still ahead — a Raspberry Pi 4 |
