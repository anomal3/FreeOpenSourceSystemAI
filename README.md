...
| **v0.7b** | A desktop a Windows user does not have to get used to: dialogs with buttons instead of `Y`/`N`, a start menu of applications rather than of `/bin`, a tray with the input language, network and clock, RU/EN keyboard layouts switched with Alt+Shift or Win+Space, a file manager that opens on a double click and has a context menu, a task manager that can end a task | in progress: С1–С8 of С9 done; the resolution changes without a reboot, a device manager shows what drives each device, window programs share one set of elements |
