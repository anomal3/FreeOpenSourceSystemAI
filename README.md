...
| **v0.7c** | C# programs and WinForms: a `.dll` built with `dotnet build` on Windows runs here unchanged, on an own .NET runtime — an IL interpreter, a precise garbage collector, an own base library and an own `System.Windows.Forms` over FreeOS windows | in progress: N1–N2 of N8 done — assemblies are read exactly as `System.Reflection.Metadata` reads them, and `dotnet hello.dll` prints Hello, World! from an unmodified `dotnet new console` project |
