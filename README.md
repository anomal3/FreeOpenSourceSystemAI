...
| **Userspace** | ELF programs in ring 3 / EL0, one address space each, preemptive scheduling, pipes, `mode`/`uid`/`gid` enforced, memory on request (`mmap`) with a per-task cap |
...
| next | Demand paging, file mapping, huge pages, SMP | planned |
