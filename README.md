...
| **Userspace** | ELF programs in ring 3 / EL0, one address space each, preemptive scheduling, pipes, `mode`/`uid`/`gid` enforced, memory on request (`mmap`), and files mapped into memory and paged in on demand |
...
| next | Huge pages, SMP; then writable file mappings and eviction under pressure | planned |
