...
pub unsafe extern "C" fn bare_main(dtb: *const u8) -> ! {
    ...
    if let (Some(base), Some(bytes)) = (framebuffer(&fdt), vram(&fdt)) {
        ...
        // Таймер гасится **до** заливки, а не после. Заливка — это десятки
        // мегабайт записей по памяти, к которой нет кэша: с выключенным MMU
        // каждая запись уходит на шину отдельной посылкой, и такой цикл легко
        // длится дольше, чем терпит сторожевой таймер. Сначала он гасился после
        // неё, и аппарат сбрасывался посреди работы — то есть опыт мерил
        // скорость записи, а не то, ради чего ставился.
