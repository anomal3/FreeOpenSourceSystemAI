...
pub unsafe extern "C" fn bare_main(dtb: *const u8) -> ! {
    ...
    let Some(fdt) = (unsafe { Fdt::from_ptr(dtb) }) else {
        ...
        // адрес экрана берутся из того же дерева. Остаётся встать — а дальше
        // сторожевой таймер, который мы не успели погасить, перезапустит
        // аппарат. Это и есть признак: см. договор ниже.
        halt()
    };
    ...
    // Сторожевой таймер гасится **последним**, и это не мелочь, а весь договор
    // с наблюдателем.
    //
    // Заводской загрузчик взводит таймер перед передачей управления и ждёт, что
    // ядро его погасит. Значит всё, что не дошло досюда, — отказ разбора,
    // исключение, зависание — кончается перезагрузкой, и аппарат сам возвращается
    // в Android: опыт можно повторять, не трогая телефон руками. А дошедшее
    // досюда останавливает машину намертво.
    //
    // Читается это так: **замер — получилось**; вернулся в Android — не дошли.
