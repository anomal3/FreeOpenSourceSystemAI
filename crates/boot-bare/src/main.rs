...
pub unsafe extern "C" fn bare_main(dtb: *const u8) -> ! {
    ...
    let Some(fdt) = (unsafe { Fdt::from_ptr(dtb) }) else {
        ...
        // остановленная машина, — и чтобы он был, таймер надо погасить вслепую.
        blind_watchdog();
        halt()
    };

    // Сторожевой таймер — сразу после разбора дерева, раньше всего остального.
    //
    // Порядок здесь и есть договор с наблюдателем, и первая его редакция была
    // неверной: таймер гасился **последним**, и тогда «код упал по дороге» и
    // «загрузчик нас вообще не запускал» давали один и тот же исход —
    // перезагрузку. То есть самый первый вопрос, на который нужен ответ,
    // оставался без ответа. Теперь наоборот: **остановленная машина означает,
    // что наш код исполнялся**, а перезагрузка — что до него не дошло.
...
pub extern "C" fn bare_fault(vector: u64, esr: u64) -> ! {
    ...
    if known.line.is_none() {
        // Линии нет — значит и рассказать об отказе нечем, и единственным
        // признаком остаётся остановленная машина. Без этого исключение,
        // случившееся до того, как мы погасили таймер, выглядело бы как
        // «загрузчик нас не запускал».
        //
        // Там, где линия есть, этого делать нельзя: адрес чужой, запись по нему
        // отдаст новое исключение, и отчёт уйдёт в бесконечный круг.
        blind_watchdog();
    }
    halt()
}
...
/// Погасить сторожевой таймер, не заглядывая в дерево.
///
/// Единственный вбитый в код адрес во всём образе, и он **не угадан**: снят с
/// самого аппарата, из его же `/proc/device-tree/toprgu@10007000`. Применяется
/// только там, где дерева нет или ему уже нельзя верить.
fn blind_watchdog() {
    const MT676X_WATCHDOG: u64 = 0x1000_7000;
    // SAFETY: адрес принадлежит регистрам устройства этой машины; запись
    // обязана быть volatile. На машине, где такого регистра нет, запись отдаст
    // исключение — и остановит нас так же, как остановил бы `halt`.
