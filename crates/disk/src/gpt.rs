...
/// Тип раздела: данные, живущие своей жизнью, — `/data`.
///
/// Отдельный от состояния тип, а не «состояние, только другое». Разница в том,
/// кто раздел переживает: состояние обязано пережить обновление системы, а
/// данные обязаны пережить **переустановку** — включая ту, где диск
/// размечается заново. Установщик поэтому такой раздел не создаёт и не трогает
/// вовсе: он появляется только тогда, когда его завёл человек.
///
/// Это же первый раздел, на котором живёт btrfs. Порядок выбран не случайно:
/// новая файловая система обкатывается там, где её потеря не мешает системе
/// загрузиться, — и переезжает на `/home` не в день, когда запись заработала.
pub const FREEOS_DATA_TYPE: Guid = Guid::new(
    0x0F7B_3A4E,
    0x2C58,
    0x4D94,
    [0x9E, 0x6A, 0x1B, 0x84, 0xC2, 0xD5, 0xA7, 0x30],
