...
fn choose_mode(gop: &mut GraphicsOutput, policy: Policy) {
    ...
    // Режим выбирается на любой машине, а не только на прошивке EDK II, и это
    // возвращение к прежнему поведению после разбирательства.
    //
    // История целиком, потому что она поучительна. На ASUS K53SD (AMI, UEFI
    // 2.0, Intel HD 3000) после смены режима экран гас навсегда, и первым
    // подозреваемым стала сама смена — запрет появился отсюда. Настоящая
    // причина нашлась позже и оказалась совсем другой: `probe_framebuffer`
    // открывал GOP **монопольно**, прошивка отключала от него графическую
    // консоль, и следующая же печать уводила загрузчик в панику (см.
    // `probe_framebuffer`). Причину починили, а запрет остался — и стоил
    // Роману рабочего стола в 800×600 на матрице 1366×768, потому что прошивка
    // стартует мелко, а лучший режим (1024×768) она предлагает.
    //
    // Страховка на случай, если где-то смена всё же гасит экран, осталась
    // прежней и проверенной: пятый пункт меню загрузчика оставляет режим
    // прошивки нетронутым.
    //
