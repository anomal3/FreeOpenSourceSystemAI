...
/// ядро, фреймбуфер и образ ФС добавляет по паре записей. Множитель в
...
/// ядра, фреймбуфер, образ ФС), и живёт на стеке — это допустимо, потому что стек
...
pub fn exit_and_jump(handoff: Handoff, entry: u64, overrides: &[Override]) -> ! {
    ...
    // образа ядра освобождён, а `Handoff` держит только сырые указатели на страницы,
