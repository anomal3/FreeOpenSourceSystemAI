...
impl Editor {
    ...
    /// Длина файла по номеру inode.
    ///
    /// Нужна тому, кто кладёт файл в готовый образ и хочет знать, тот ли файл
    /// там уже лежит. Одного имени для этого мало: имя переживает смену
    /// содержимого, и «уже положили» тогда означало бы «лежит что-то с таким
    /// же именем» — а стенд после этого ждал бы от гостя число, посчитанное по
    /// другому файлу.
    pub fn size_of(&mut self, dev: &mut dyn BlockDevice, number: u32) -> Result<u64> {
        Ok(self.read_inode(dev, number)?.size)
    }
    ...
    ///
    /// Содержимое при этом не читается и не копируется вовсе — переезжает
    /// **запись каталога**. Разница видна на файле в гигабайт, и она же
    /// объясняет, почему переименование не бывает наполовину выполненным: inode
    /// один и тот же до и после.
    ///
    /// # Порядок двух действий
    ///
    /// Сначала появляется новое имя, потом исчезает старое. Обратный порядок
    /// оставил бы после сбоя питания inode без единого имени — потерянный файл,
    /// который найдёт только `fsck`. При этом же порядке худшее, что бывает, —
    /// файл, видимый под двумя именами: обе записи целы, обе ведут на живой
    /// inode, и проверка тома чинит это, поправив счётчик ссылок.
    ///
    /// # Чего он не делает
    ///
    /// Не перезаписывает занятое имя: `rename` в POSIX молча заменяет приёмник,
    /// и это ровно то место, где программа теряет файл, о котором не знала.
    /// Здесь это отказ [`Error::Exists`], и решать, удалять ли, обязан тот, кто
    /// просил.
    ///
    /// Не переносит **каталог** в другого родителя: внутри каталога лежит «..»,
    /// и оставить его указывающим на прежнего родителя значило бы испортить
    /// дерево, а поправить — переписать заодно счётчики ссылок обоих каталогов.
    /// Переименование каталога на месте разрешено: там родитель не меняется.
    pub fn rename(
        &mut self,
        dev: &mut dyn BlockDevice,
        old_parent: u32,
        ...
        let Some((number, kind)) = self.lookup(dev, old_parent, old_name)? else {
            return Err(Error::NotFound);
        };
        if self.lookup(dev, new_parent, new_name)?.is_some() {
            return Err(Error::Exists);
        }
        if kind == DIR_TYPE_DIRECTORY && old_parent != new_parent {
            return Err(Error::Unsupported);
        }
        // Переименование в себя же — не работа, но и не ошибка: спрашивающий
        // получил ровно то, чего просил.
        if old_parent == new_parent && old_name == new_name {
            return Ok(());
        }

        self.mark_dirty(dev)?;
        self.add_entry(dev, new_parent, new_name, number, kind)?;
        self.remove_entry(dev, old_parent, old_name)?;
        // Счётчик ссылок не трогается: имя переехало, а не удвоилось. Каталог
        // при этом остаётся в том же родителе (проверено выше), поэтому «..» и
        // ссылки родителей тоже неизменны.
        Ok(())
    }
    ...
    /// Взять под своё крыло inode, который уже существует, но потерял имя.
    ///
    /// Нужно ровно одному вызывающему — проверке тома ([`crate::check`]),
    /// которая переносит найденное в `/lost+found`. Обычная запись файла сюда
    /// не ходит: там inode создаётся вместе с записью каталога, и разделять
    /// эти два действия незачем.
    ///
    /// Число ссылок ставится в единицу (для каталога — в два, считая «.»), а не
    /// увеличивается: посчитанное проверкой значение и есть правда о томе, а
    /// то, что лежало в inode, уже признано неверным. Родительский каталог
    /// получает свою ссылку от «..», если приёмыш — каталог.
    pub fn adopt_orphan(
        &mut self,
        dev: &mut dyn BlockDevice,
        parent: u32,
        ...
        check_name(name)?;
        let kind = if is_dir { DIR_TYPE_DIRECTORY } else { DIR_TYPE_REGULAR };
        self.add_entry(dev, parent, name, number, kind)?;

        let mut inode = self.read_inode(dev, number)?;
        inode.links = if is_dir { 2 } else { 1 };
        self.write_inode(dev, number, &inode)?;
        if is_dir {
            // У каталога-приёмыша «..» теперь указывает не туда, куда раньше;
            // счётчик ссылок нового родителя это учитывает, а саму запись «..»
            // чинить нечем — она может быть испорчена ровно так же, как всё
            // остальное в потерянном каталоге, и её починка потребовала бы
            // решения, которого у проверки нет.
            self.bump_links(dev, parent, 1)?;
        }
        Ok(())
    }
