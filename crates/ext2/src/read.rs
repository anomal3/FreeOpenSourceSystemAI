...
impl Ext2 {
    ...
    pub fn inode(&self, dev: &mut dyn BlockDevice, number: u32) -> Result<Inode> {
        ...
        // Файл больше тома — это испорченный инод, а не большой файл. Проверка
        // стоит здесь, у единственного места, где размер появляется, а не у
        // каждого читающего: по этому числу считается, сколько блоков
        // перебрать, и чужой том с размером в два гигабайта на двух мегабайтах
        // заставлял чтение работать секундами, прежде чем отказать (найдено
        // фаззером, цель `ext2`). Разреженный файл законно занимает меньше
        // блоков, чем обещает размером, — но не больше, чем есть весь том.
        if size > self.geometry.volume_bytes() {
            return Err(Error::Corrupt);
        }
