//! Куча объектов.
//!
//! Объекты живут до конца программы: сборщика мусора ещё нет, он приходит в
//! фазе N3d. Строки `ldstr` не выделяются заново (см. `Vm::load_string`), а
//! склейки в цикле и копии структур растят кучу — и об этом говорит число
//! объектов в строке завершения `/bin/dotnet`.
