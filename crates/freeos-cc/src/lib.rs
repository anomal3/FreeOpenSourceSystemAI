...
/// `-fPIE` — программа позиционно-независимая (ASLR, часть 2): ядро кладёт её
/// на случайную страницу окна, а перемещения применяет стартовый код
/// (`libc/freeos/start-<арх>.s`). До этого здесь стояли `-fno-pic` и, на
/// x86-64, `-mcmodel=large` — программа жила по абсолютному адресу 512 ГиБ, а
/// малая модель кода до него не дотягивалась. Библиотеки (picolibc, zlib, Lua)
/// собираются теми же флагами: объектник без PIC в позиционно-независимую
/// программу не компонуется.
///
/// `-fstack-protector-strong` — канарейка в каждой функции с массивом на стеке
/// или с локальной переменной, чей адрес утёк: тот же выбор, что у
/// дистрибутивов Linux, и тот же, что у программ на Rust
/// (`-Z stack-protector=strong` в `xtask::build`). Эталон компилятор читает по
/// символу `__stack_chk_guard`, а лежит тот в странице процесса — странице
/// только на чтение, которую ядро заполняет случайными байтами до входа в
/// программу; символ на неё ставит компоновочный сценарий, и программе
/// делать ничего не нужно. `-mstack-protector-guard=global` — обязательно и
/// именно из-за триплета: услышав «linux», clang на x86-64 читал бы эталон из
/// `%fs:0x28`, то есть из TLS, которого у программы нет. Падение зовёт
/// `__stack_chk_fail` из стартового кода (`libc/freeos/crt0.c`).
///
...
/// Как компонуется позиционно-независимая программа: от нуля (`user.ld`
/// переключается символом `__freeos_pie`), с входом в стартовый код, который
/// применяет перемещения до `_start` (`libc/freeos/start-<арх>.s`, лежит в
/// `crt0.o`). Одно место на `freeos-cc` и на `xtask::cbuild`.
pub const LINK_PIE: [&str; 4] = ["-pie", "--defsym=__freeos_pie=1", "-e", "__freeos_entry"];
