...
extern "C" fn resume_on_kernel_stack(boot_info: usize) -> ! {
    ...
    // Остальные процессоры — сразу, как только появились часы: ожидание их
    // запуска меряется временем, а стек каждого берётся из кучи. Раньше, чем
    ...
    // ФС корневая.
    time::adopt_timezone();
    // Раскладка — из того же файла, теми же словами, что записал установщик.
    input::keymap::adopt();
    ...
    start_graphics(&info, have_input);
    ...
    // Постоянный адрес — сразу после карты и **до** служб. Порядок обязателен:
    // `/bin/dhcp` смотрит в тот же файл и, увидев `mode=static`, отступается, а
    // аренда, приехавшая на секунду позже применения, перезаписала бы то, что
    // человек задал сам.
    net::persist::adopt();
    ...
    // Перепись шины — последней: к этому моменту отметились все драйверы, и
    // приговор «драйвера нет» означает именно это, а не «очередь ещё не дошла».
    // На своей машине она избыточна, на чужой — единственный способ узнать, что
    // в ней стоит, не разбирая корпус.
    devices::log_census();
...
fn start_graphics(info: &BootInfo, have_input: bool) {
    kprintln!();
    kprintln!("---- display ----------------------------------------------------");

    hold_boot_log(info);

    // Рабочий стол на машине, которой нечем управлять, — это картинка, на
    // которую нельзя нажать. Журнал на её месте полезнее, и не отвлечённо:
    // экран у телефона единственный канал наружу, а композитор его забирает
    // насовсем.
    //
    // Есть и вторая причина, найденная опытом. Отдача экрана и отрисовка стола
    // — это секунды сплошной работы с некэшируемой памятью, а служебные задачи
    // запускаются только после них. В это окно шину USB опрашивать некому, и
    // хост, спросивший в него, объявляет разрыв и больше не возвращается: обмен
    // по кабелю переживал загрузку ровно до появления стола.
    //
    // Условие снимется само. Заработает тачскрин — появится источник событий, и
    // стол поедет как раньше, без единой правки здесь.
    if !have_input {
        kprintln!(
            "  compositor  : not started -- no input on this machine; the log keeps the screen"
        );
        return;
    }
...
fn hold_boot_log(info: &BootInfo) {
    ...
    // Предел, а не срок. Журнал держится, пока обмен по кабелю не наладится, и
    // это правильный порядок: рабочий стол на машине без ввода не нужен никому,
    // а журнал — единственное, что она умеет рассказать о себе. Пять минут
    // стоят здесь на случай, если наладить не удастся вовсе: без них ядро
    // осталось бы на загрузочном экране навсегда.
    ...
    // Итог обмена по кабелю досказывается не в конце, а задолго до него. В
    // конце было бы поздно: следующее, что делает ядро, — отдаёт экран
    // рабочему столу, и строка мелькнула бы, не успев быть прочитанной. А
    // раньше первых секунд её не напечатать: пока неизвестно, заговорил ли с
    // нами хост, — ради этого удержание и существует.
    //
    // Отчёт повторяется, и это не расточительство. Одно значение счётчика
    // говорит только «сколько», а несколько подряд — «растёт ли», и это разные
    // ответы: остановившийся счётчик означает, что хост махнул рукой, а
    // растущий — что он всё ещё спрашивает, а мы всё ещё молчим. Снимок экрана
    // делается руками и один раз, так что динамика обязана уместиться в него.
    const REPORT_EVERY: u64 = 10;

    let started = time::uptime_ms();
    let until = started + HOLD_SECONDS * 1000;
    let mut next_report = started + REPORT_EVERY * 1000;

    while time::uptime_ms() < until {
        if time::uptime_ms() >= next_report {
            // Отчёт сам говорит, дождались ли мы своего. Пока не дождались —
            // держим журнал: снимок экрана делается руками и один раз, и
            // человек не обязан попадать в него по секундомеру.
            if arch::report_boot_devices() {
                break;
            }
            next_report += REPORT_EVERY * 1000;
        }
        // Эти же двадцать секунд — окно, в которое компьютер успевает
        // перечислить устройство. Планировщик ещё не запущен, служебной задачи
        // USB тоже нет, а хост начинает спрашивать сразу, как увидел подтяжку.
        // Просто ждать здесь значило бы промолчать в ответ на все его запросы
        // и получить «устройство не опознано» ровно один раз — второго захода
        // хост не делает.
        arch::poll_boot_devices();
        core::hint::spin_loop();
    }
}
...
fn panic(info: &PanicInfo<'_>) -> ! {
    // Остальные процессоры останавливаются до сообщения: иначе их вывод рвал бы
    // его на части, а их работа продолжалась бы на состоянии, которое привело к
    // панике.
    smp::stop_others();
