...
extern "C" fn resume_on_kernel_stack(boot_info: usize) -> ! {
    ...
    // Канарейка ядра — сразу за источником случайности и ни строкой позже: до
    // этого места эталон известен наперёд (см. `guard`), а каждая следующая
    // подсистема разбирает всё более чужие байты.
    //
    // Порядок здесь — часть договора, а не оформление. Значение считается
    // заранее, отдельной функцией: она возвращается, и её собственная
    // канарейка обязана сойтись со **старым** эталоном. Пишет эталон голая
    // функция без пролога, и зовётся она отсюда — из функции, которая не
    // возвращается. Прерывания на эти две команды выключены: обработчик,
    // заставший замену посередине, объявил бы переполнение на ровном месте.
    let (canary, source) = guard::fresh();
    arch::interrupts::disable();
    // SAFETY: три условия из `guard::arm` выполнены здесь и видны глазом —
    // эта функция объявлена `-> !`, прерывания только что выключены, и другого
    // вызова в ядре нет (`smp::start` ниже по тексту, второго процессора пока
    // тоже нет).
    unsafe { guard::arm(canary) };
    guard::mark_armed();
    arch::interrupts::enable();
    kprintln!("  canary      : kernel stack guard armed from {}", source.name());
...
fn hold_boot_log(info: &BootInfo) {
    // Без кадрового буфера держать нечего: экрана, на котором журнал ловят
    // снимком, нет. Так грузится QEMU `virt` по договору Linux — сценарий стенда
    // `devicetree` (фаза 51), — и тридцать секунд ожидания там не дают ничего.
    if info.device_tree == 0 || !info.framebuffer.is_present() {
        return;
    }
...
fn mount_disk_root(info: &BootInfo) {
    ...
    // Разводка прерываний читается до перебора шины: драйверу, который
    // захочет линию, она понадобится в момент подключения, а не потом.
    irq::routing::init();
...
fn start_interrupts() {
    ...
    while irq::ticks() < target {
        ...
        //
        // Предел считается в витках, а не во времени, и иначе здесь нельзя:
        // время — это и есть то, чего мы ждём. Зато решение по исчерпанному
        // пределу принимается по существу. Строка утверждает «прерывания не
        // доходят до процессора», и проверять надо ровно это — пришёл ли
        // ХОТЬ ОДИН тик, а не набралось ли их на полсекунды. Разница не
        // теоретическая: в release виток пустой, два миллиарда витков на
        // AArch64 проходят быстрее полусекунды, и 41 из 151 журнала release
        // нёс эту тревогу при исправном таймере (19.09.2026).
        if spins > 2_000_000_000 {
            if irq::ticks() == start {
                kprintln!("  timer       : NO TICKS -- interrupts are not reaching the CPU");
            } else {
                kprintln!(
                    "  timer       : {} tick(s) while waiting for {}; the counter runs, just slowly",
                    irq::ticks() - start,
                    target - start
                );
            }
