...
pub fn handle(fault: Fault, ctx: TrapContext) -> ! {
    // Остальные процессоры — первыми: их вывод иначе перемешался бы с отчётом, а
    // их работа продолжилась бы поверх состояния, которому уже нельзя верить.
    crate::smp::stop_others();
    kprintln!();
    kprintln!("*** CPU FAULT ***");
