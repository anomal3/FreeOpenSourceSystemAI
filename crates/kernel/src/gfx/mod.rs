//! Рисование: прямоугольники, поверхности в памяти и вывод их на экран.
//!
//! # Две разные памяти
//!
...
//!
//! Отсюда же вытекает устройство композитора ([`crate::ui`]): экран — это только
//! приёмник, и всё, что на него попадает, собирается заранее.
//!
//! # Почему поверхности хранят уже упакованные пиксели
//!
//! Формат пикселя (порядок каналов) задаёт прошивка, и он один на всю машину.
//! Если поверхность хранит цвет в виде (R, G, B), то каждый вывод на экран — это
//! упаковка миллиона пикселей; если она хранит уже упакованные слова, вывод
//! становится копированием. Поэтому формат запоминается один раз при
//! [`init`], а [`Color::pixel`] обращается к нему.
//!
//! Цена — глобальное состояние. Плата за альтернативу выше: формат пришлось бы
//! протаскивать в каждую функцию рисования и в каждую структуру, которая рисует.

// Геометрия заводится небольшим полным набором: `contains` — это проверка
// попадания, без которой не обойдётся ни одна реакция на указатель, `shrink` —
// отступы. Оставить только то, чем пользуется сегодняшний композитор, значит
// дописывать по одному методу на каждый следующий элемент интерфейса. Так же
// поступают `mm` и `pci`.
#![allow(dead_code)]
...
/// Формат пикселя, запомненный при [`init`].
///
...
/// Запомнить формат пикселя машины. Вызывается один раз при создании экрана.
...
impl Color {
    ...
    ///
    /// До [`init`] возвращает ноль: рисовать ещё некуда, и чёрный — самый
    /// безобидный ответ.
    #[must_use]
...
/// Поверхность в обычной памяти: то, во что рисуют окна.
///
...
impl Surface {
    ...
    /// это несколько мегабайт, и отказ здесь совершенно реален. Паниковать из-за
    /// этого нельзя — ядро обязано продолжить работу без графики.
    #[must_use]
    pub fn new(width: u32, height: u32, fill: Color) -> Option<Self> {
        ...
        // ошибкой, а не уйти в `handle_alloc_error` и остановить ядро.
...
impl Screen {
    /// Создать экран по описанию от загрузчика.
    ///
    ...
    pub fn blit(&self, surface: &Surface, dst: (i32, i32), src_rect: Rect) {
        ...
        for row in 0..visible.h {
            ...
            let start = ((visible.y as u32 + row) as usize) * (self.stride as usize)
                + visible.x as usize;
