...
/// `IA32_PAT` — таблица типов памяти: восемь входов по байту, каждый задаёт тип
/// для своей комбинации битов `PAT`, `PCD`, `PWT` в записи таблицы страниц.
const IA32_PAT: u32 = 0x277;
/// Признак поддержки `PAT` — `CPUID.01H:EDX`, бит 16.
const CPUID_EDX_PAT: u32 = 1 << 16;
/// Тип памяти «write combining» в терминах таблицы.
const PAT_TYPE_WC: u64 = 0x01;
/// Вход, который мы переписываем: номер 1, то есть комбинация `PWT = 1`,
/// `PCD = 0`, `PAT = 0`.
///
/// Выбран не произвольно. По умолчанию этот вход означает write-through, и
/// **никакое** наше отображение его не выбирает: `PWT` ядро ставит только
/// вместе с `PCD` (память устройства, вход 3). Значит переписав его, мы не
/// меняем смысл ни одной уже существующей записи — а это единственный способ
/// трогать таблицу типов памяти безопасно.
const PAT_WC_SLOT: u32 = 1;
...
/// Переписан ли вход таблицы типов памяти под write-combining.
///
/// Атомик, а не «сделали и забыли»: запись таблицы страниц собирается до того,
/// как первый разбуженный процессор доберётся до своей настройки, и строить
/// записи, опирающиеся на непереписанную таблицу, было бы неверно. Пока флага
/// нет, кадровый буфер отображается как память устройства — медленно, но верно.
static PAT_READY: AtomicBool = AtomicBool::new(false);
...
/// Физический адрес PML4 **ядра**.
///
/// Пока пользовательских пространств не было, «корень ядра» и «то, что лежит в
/// CR3» означали одно и то же, и корень читался прямо из регистра. С Phase 12b
/// это перестало быть верным: на время работы программы в CR3 стоит её корень,
/// и `map_active`, прочитав регистр, добавила бы отображение устройства в
/// таблицы программы — то есть в дерево, которое будет разобрано при её
/// завершении. Отображение исчезло бы вместе с ней, а обнаружилось бы на первом
/// же обращении к регистрам контроллера, ничем не связанном по времени с
/// запуском программы.
///
/// Ноль означает «таблицы ядра ещё не активированы»; в этом случае корень
/// по-прежнему берётся из CR3 — до [`AddressSpace::activate`] там стоит дерево
/// прошивки, и другого ответа просто нет.
static KERNEL_ROOT: AtomicU64 = AtomicU64::new(0);

/// Физический адрес корневой таблицы ядра.
#[must_use]
...
impl AddressSpace for PageTable {
    fn new(alloc: &mut impl FrameAllocator) -> Result<Self, MapError> {
        ...
        enable_nx();
        // Таблицу типов памяти надо переписать до первой записи, ссылающейся на
        // её вход: иначе кадровый буфер получит отображение, которое значит не
        // то, что задумано (см. `enable_write_combining`).
    ...
    unsafe fn activate(&self) {
        ...
        // Корень запоминается до загрузки регистра, а не после: между этими
        // двумя строками ядро уже исполняется на новых таблицах, и прерывание,
        // пришедшее ровно там, застало бы `kernel_root` без ответа.
        KERNEL_ROOT.store(self.root.as_u64(), Ordering::Relaxed);
...
fn leaf_bits(phys: PhysAddr, flags: PageFlags) -> u64 {
    ...
    if flags.contains(PageFlags::WRITE_COMBINE) && PAT_READY.load(Ordering::Relaxed) {
        // `PWT` без `PCD` выбирает вход 1 таблицы типов памяти, а его мы
        // перепрограммировали в write-combining (см. `enable_write_combining`).
        // Пока таблица не тронута, этот вход означает write-through — тоже
        // кешируемый режим, и экран от него не пострадает.
        bits |= ENTRY_WRITE_THROUGH;
    } else if flags.contains(PageFlags::DEVICE) {
...
/// Взять в работу дерево таблиц **ядра**.
///
...
pub unsafe fn active_address_space() -> PageTable {
    // Именно [`kernel_root`], а не CR3: пока исполняется пользовательская
    // программа, в регистре стоит её корень, и правкой «активного» дерева мы
    // отредактировали бы таблицы, живущие до её завершения.
    PageTable { root: kernel_root(), phys_offset: Cell::new(PHYS_MAP_BASE) }
}

// --- Адресные пространства программ -------------------------------------------

/// Взять в работу дерево с заданным корнем.
///
/// Отличается от [`active_address_space`] только тем, что корень задаёт
/// вызывающий: так пространство программы правится, пока процессор работает на
/// таблицах ядра.
///
/// # Safety
///
/// * `root` обязан быть корнем дерева, построенного этим модулем, и оставаться
///   живым всё время жизни возвращённого значения;
/// * таблицы ядра должны быть активны — записи правятся через прямое
///   отображение;
/// * пока экземпляр жив, никто другой не должен править то же дерево.
pub unsafe fn space_at(root: PhysAddr) -> PageTable {
    PageTable { root, phys_offset: Cell::new(PHYS_MAP_BASE) }
}

/// Виртуальный адрес записи `index` таблицы `table` в прямом отображении.
fn table_entry(table: PhysAddr, index: usize) -> *mut Entry {
    debug_assert!(index < ENTRIES_PER_TABLE);
    ((table.as_u64() as usize + PHYS_MAP_BASE) as *mut Entry).wrapping_add(index)
}

/// Создать адресное пространство программы поверх ядерного.
///
/// Корневая таблица — копия ядерной, из которой вычеркнута одна запись:
/// `window_slot`, отведённый под память программы. Копия, а не пустая таблица,
/// по простой причине: код ядра исполняется по тем же адресам и после
/// переключения `CR3` — обработчик системного вызова, обработчик прерывания и
/// стек ядра обязаны остаться на месте, иначе первый же `int 0x80` из программы
/// уйдёт в никуда. Прав программе это не даёт: записи ядра не помечены
/// пользовательскими, и проверку делает MMU.
///
/// Копируются **значения** записей, то есть нижележащие таблицы у ядра и у
/// программы общие. Отсюда единственное ограничение: отображение, для которого
/// ядру пришлось бы завести **новую** запись верхнего уровня (то есть занять
/// пустые 512 ГиБ), в уже созданных пространствах не появится. Все окна ядра —
/// прямое отображение, куча, стек, окно DMA — заводятся при загрузке, задолго до
/// первого запуска программы, поэтому на практике этот случай не наступает; а
/// сама правка ядерного дерева, пока программа работает, направлена в него же
/// (см. [`kernel_root`]).
///
/// # Safety
///
/// Таблицы ядра должны быть активны: и копия, и оригинал адресуются через
/// прямое отображение.
pub unsafe fn new_user_space(
    ...
    for index in 0..ENTRIES_PER_TABLE {
        // Запись окна обнуляется, а не копируется: в дереве ядра её быть не
        // должно вовсе, но если она там почему-то окажется, программа получила
        // бы чужую память вместо своей — и обнаружилось бы это не сразу.
        let entry = if index == window_slot {
            Entry(0)
        } else {
        ...
        // SAFETY: см. выше; кадр только что выдан аллокатором и никому больше
        // не принадлежит.
        unsafe { ptr::write_volatile(table_entry(root, index), entry) };
    }
...
///
/// Нужно не отладке, а проверке утверждений: «окно программы в пространстве
/// ядра отсутствует» — это ровно `None` для [`kernel_root`], и сказать это
/// иначе, чем пройдя по таблицам, нельзя.
#[must_use]
pub fn translate(root: PhysAddr, virt: VirtAddr) -> Option<(PhysAddr, PageFlags)> {
    ...
    // SAFETY: `table` — листовая таблица, полученная спуском выше.
    let leaf = unsafe { ptr::read_volatile(table_entry(table, virt.table_index(LEVEL_PT))) };
    if !leaf.is_present() {
        return None;
    }
    Some((leaf.addr(), leaf_flags(leaf.0)))
}
...
/// # Safety
///
/// `root` обязан быть корнем дерева, доступного через прямое отображение, а
...
pub unsafe fn unmap(root: PhysAddr, virt: VirtAddr) -> Option<(PhysAddr, usize)> {
    ...
    let slot = table_entry(table, virt.table_index(LEVEL_PT));
    // SAFETY: `slot` — запись листовой таблицы, полученной спуском выше.
    ...
    // SAFETY: тот же слот той же таблицы. Запись обнуляется целиком: гасить
    // один бит присутствия значило бы оставить в таблице адрес кадра, который
    ...
    // означает отдать его вместе с работающей дорогой к нему.
    // SAFETY: `invlpg` не обращается к памяти и не может отказать.
    unsafe { invlpg(virt) };
...
fn leaf_flags(bits: u64) -> PageFlags {
    // Читаема любая присутствующая страница: отдельного бита «чтение
    // запрещено» на x86-64 нет.
    let mut flags = PageFlags::READ;
    if bits & ENTRY_WRITABLE != 0 {
        flags |= PageFlags::WRITE;
    }
    if bits & ENTRY_USER != 0 {
        flags |= PageFlags::USER;
    }
    if bits & ENTRY_NO_EXECUTE == 0 {
        flags |= PageFlags::EXEC;
    }
    if bits & ENTRY_CACHE_DISABLE != 0 {
        flags |= PageFlags::DEVICE;
    }
    flags
}

/// Переключить процессор на дерево программы.
///
/// # Safety
///
/// `root` обязан быть корнем, полученным из [`new_user_space`], то есть
/// содержать все отображения ядра: код, стек и обработчики продолжают
/// исполняться сразу после записи регистра.
pub unsafe fn activate_space(root: PhysAddr) {
    // SAFETY: условие делегировано вызывающему. Запись CR3 сбрасывает TLB
    // целиком — глобальных страниц ядро не заводит (см. `leaf_bits`), поэтому
    // трансляции покинутого пространства не переживут переключение.
...
/// # Safety
///
/// Вызывать только после [`AddressSpace::activate`]: до неё корня ядра ещё не
/// существует.
pub unsafe fn activate_kernel_space() {
    // SAFETY: см. контракт функции; дерево ядра отображает исполняющийся код и
    // стек по тем же адресам, что и покидаемое.
    unsafe { write_cr3(kernel_root()) };
}

/// Разобрать адресное пространство программы: вернуть в пул всё, что лежит под
/// `window_slot`, и саму корневую таблицу.
///
/// Освобождается **только** поддерево окна. Остальные записи корня — копии
/// ядерных, и таблицы под ними общие с ядром; освободить их значило бы вынуть
/// память из-под работающей системы.
///
/// Возвращает `(таблицы, страницы)`: сколько кадров ушло под сами таблицы (с
/// учётом корня) и сколько под страницы программы. Разделение не для красоты —
/// по нему видно, что окно разобрано целиком, а не наполовину.
///
/// # Safety
///
/// * `root` получен из [`new_user_space`] и больше не активен ни на одном
///   процессоре;
/// * таблицы ядра активны (доступ идёт через прямое отображение).
pub unsafe fn free_user_space(
    ...
    if window_slot < ENTRIES_PER_TABLE {
        ...
        if entry.is_present() && !entry.is_huge() {
            ...
            // таблицу уровня PDPT; спуск по ней корректен.
            unsafe { free_subtree(entry.addr(), LEVEL_PML4 - 1, alloc, &mut tables, &mut pages) };
        }
        // Запись стирается до освобождения корня: кадр вернётся в пул и будет
        // выдан заново, а «почти правильная» ссылка на уже освобождённое
        // поддерево — худший вид мусора в таблице страниц.
        // SAFETY: тот же слот того же кадра.
        unsafe { ptr::write_volatile(slot, Entry(0)) };
    }

    // SAFETY: корень выдан аллокатором в `new_user_space`, больше не активен и
    // ни в одном дереве на него ссылок нет.
...
unsafe fn free_subtree(
    ...
    for index in 0..ENTRIES_PER_TABLE {
        ...
        let entry = unsafe { ptr::read_volatile(table_entry(table, index)) };
        if !entry.is_present() {
            continue;
        }
        ...
        if level == LEVEL_PT {
            // SAFETY: листовая запись окна ссылается на кадр, выделенный под
            // страницу программы, и больше нигде не используется.
            unsafe { alloc.free(entry.addr()) };
            *pages += 1;
        } else {
            // SAFETY: промежуточная запись ссылается на таблицу следующего
            // уровня того же дерева.
            unsafe { free_subtree(entry.addr(), level - 1, alloc, tables, pages) };
        }
    }
    // SAFETY: все ссылки из этой таблицы обработаны, ссылок на неё саму больше
    // нет — вызывающий стирает свою запись сразу после возврата.
...
/// Переписать вход 1 таблицы типов памяти в write-combining.
///
/// # Зачем
///
/// Чтобы экран можно было отображать памятью, в которую разрешено писать
/// пачками. Без этого кадровый буфер приходится держать некешируемым, и каждая
/// запись четырёх байт уходит в шину отдельной посылкой: на ноутбуке с матрицей
/// 1366×768 это больше миллиона посылок на кадр, и перетаскивание окна отстаёт
/// от мыши на глаз. В эмуляторе разницы нет — там за видеопамятью стоит обычная
/// память хоста, поэтому дефект такого рода стенд не показывает никогда.
///
/// # Почему это безопасно
///
/// Переписывается **единственный** вход, которого не выбирает ни одно наше
/// отображение (см. [`PAT_WC_SLOT`]). Остальные семь остаются как были, то есть
/// смысл всех существующих записей таблиц страниц не меняется.
///
/// Вызывается на каждом процессоре: `IA32_PAT` — регистр ядра, а не машины, и
/// запрограммированный только на загрузочном он дал бы быстрый экран на одном
/// ядре и медленный на остальных.
pub(super) fn enable_write_combining() {
    // Поддержку надо спросить: на процессоре без `PAT` запись в этот MSR даёт
    // #GP, а обработчиков исключений на этой стадии ещё нет.
    if cpuid(1, 0).edx & CPUID_EDX_PAT == 0 {
        return;
    }

    // SAFETY: поддержка подтверждена через CPUID. Меняется один байт таблицы —
    // тот, который не выбирает ни одна наша запись; остальные сохраняются.
    unsafe {
        let shift = PAT_WC_SLOT * 8;
        let value = (rdmsr(IA32_PAT) & !(0xFFu64 << shift)) | (PAT_TYPE_WC << shift);
        wrmsr(IA32_PAT, value);
    }
    PAT_READY.store(true, Ordering::Relaxed);
}
...
fn map_physical_memory(
    ...
    for index in 0..count as usize {
        ...
        if matches!(region.kind, KIND_RESERVED | KIND_ACPI_NVS) {
            flags |= PageFlags::DEVICE;
        }
        // Кадровый буфер — особый случай: кешировать его нельзя (за ним экран,
        // а не память), но и держать некешируемым разорительно. Ему нужно
        // объединение записей, и оно даёт разницу в разы на живой машине
        // (см. [`PageFlags::WRITE_COMBINE`]).
        if region.kind == KIND_FRAMEBUFFER {
            flags |= PageFlags::WRITE_COMBINE;
        }
