...
const FCR_ENABLE: u8 = 0x01; // включить оба FIFO
const FCR_CLEAR_RX: u8 = 0x02; // очистить приёмное
const FCR_CLEAR_TX: u8 = 0x04; // очистить передающее

/// Порог приёмного FIFO: прерывать на **каждом** байте.
///
/// Здесь стоял порог в 14 байт (`0xC0`) — «как принято», ради меньшего числа
/// прерываний. Цена такого умолчания видна, только если посчитать: на пороге 14
/// до переполнения остаётся два байта, то есть 170 мкс линии. Ядро же печатает
/// строку с запрещёнными прерываниями, и строка оболочки — это до 512 байт,
/// сорок миллисекунд. Порог 14 означал, что прерывание приходит тогда, когда
/// спасать уже почти нечего.
///
/// На пороге 1 запас — пятнадцать байт вместо двух, а лишние прерывания стоят
/// ровно столько же, сколько символ, набранный человеком, то есть ничего.
const FCR_TRIGGER_1: u8 = 0x00;
...
/// `IA32_FS_BASE` — база, от которой третье кольцо адресует своё хранилище.
///
/// Ядро сегмент `FS` не использует вовсе, поэтому регистр принадлежит целиком
/// программе. Он один на процессор, а не один на задачу, — значит переставлять
/// его обязано переключение контекста.
const IA32_FS_BASE: u32 = 0xC000_0100;

/// Что записано в `IA32_FS_BASE` на каждом процессоре прямо сейчас.
///
/// Нужно, чтобы не писать MSR на каждом переключении: запись в него стоит на
/// порядок дороже сравнения, а у подавляющего большинства задач база нулевая и
/// не меняется. Кеш по процессору, а не по задаче: регистр принадлежит железу.
static FS_BASE_NOW: [core::sync::atomic::AtomicU64; crate::smp::MAX_CPUS] =
    [const { core::sync::atomic::AtomicU64::new(0) }; crate::smp::MAX_CPUS];

/// Поставить базу хранилища потока, если она отличается от нынешней.
pub fn set_user_tls(base: u64) {
    let cpu = crate::smp::cpu();
    if FS_BASE_NOW[cpu].swap(base, core::sync::atomic::Ordering::Relaxed) == base {
        return;
    }
    // SAFETY: `IA32_FS_BASE` существует на всяком x86-64; запись в него меняет
    // только базу сегмента `FS`, которым ядро не пользуется.
...
///
/// На этой архитектуре таких нет. Функция существует ради общего интерфейса с
/// AArch64, где телефон в первые секунды жизни отвечает хосту по USB сам, без
/// планировщика: линии наружу у него нет, и этот кабель — единственный способ
/// узнать, чем кончилась загрузка.
pub fn poll_boot_devices() {}

/// Досказать в журнал то, что стало известно только к концу загрузки.
///
/// На этой архитектуре досказывать нечего: журнал никуда не денется, его можно
/// перечитать в файле serial когда угодно. Функция существует ради общего
/// интерфейса с AArch64, где журнал живёт на экране считаные секунды.
...
impl SerialDevice for Serial {
    fn init(&mut self) {
        ...
        unsafe {
            ...
            outb(base + REG_FCR, FCR_ENABLE | FCR_CLEAR_RX | FCR_CLEAR_TX | FCR_TRIGGER_1);
...
/// Забрать из приёмника COM1 всё, что там есть. Возвращает число байт в `out`.
///
...
pub fn drain_serial_rx(out: &mut [u8]) -> usize {
    ...
    while taken < out.len() {
        ...
        out[taken] = unsafe { inb(COM1 + REG_DATA) };
        taken += 1;
    }
    taken
}
