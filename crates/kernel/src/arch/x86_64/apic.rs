...
/// Контроллер xHCI. Приходит по MSI-X, а не через I/O APIC: устройство лежит на
/// PCIe, где линий прерываний нет вовсе, а есть запись в память.
pub const VECTOR_XHCI: u8 = 0x23;

/// Событие ACPI — то, что спецификация называет SCI. Номер входа берётся из
/// FADT (`SCI_INT`) и на разных машинах разный, вектор же наш собственный.
///
/// Из всего, что приходит этим вектором, ядро разбирает одну кнопку питания
/// (см. [`super::power`]); остальные источники событий описаны в AML, которого
/// мы не читаем.
pub const VECTOR_SCI: u8 = 0x24;

/// Куда и что должно записать устройство, чтобы вызвать вектор `vector`.
///
/// Адрес MSI на x86-64 — это не адрес памяти, а команда контроллеру,
/// закодированная в адресе: `0xFEE` в старших разрядах опознаётся локальным
/// APIC, а номер целевого ядра лежит в битах 19:12. Данные несут номер вектора;
/// нули в старших разрядах означают доставку `Fixed` по физическому адресату,
/// то есть «именно этому ядру, именно этим вектором».
///
/// Отсюда и вся разница с I/O APIC: маршрут задаётся не таблицей в чипсете, а
/// самим устройством, поэтому разбор `_PRT` из ACPI (то есть интерпретатор AML)
/// не нужен. Ради этого MSI-X и выбран.
#[must_use]
pub fn msi_target(vector: u8) -> (u64, u32) {
    const MSI_ADDRESS_BASE: u64 = 0xFEE0_0000;
    const MSI_DESTINATION_SHIFT: u32 = 12;

    let address = MSI_ADDRESS_BASE | (u64::from(local_id()) << MSI_DESTINATION_SHIFT);
...
///
/// Режим, в котором APIC передала прошивка, и сменить его самим нельзя. Сначала
/// ядро брало x2APIC всюду, где его объявляет CPUID: регистры через MSR, без
/// окна MMIO. Но включать x2APIC — решение не только ядра. Обработчики SMM
/// старых прошивок обращаются к APIC через окно `0xFEE0_0000`, а в режиме
/// x2APIC это окно выключено. Найдено на ASUS K53SD (AMI, UEFI 2.0, Sandy
/// Bridge): пока работал один процессор, система жила; как только
/// просыпались остальные, машина намертво вставала за заголовком «processors»
/// — то после строки `cpu 1 online`, то до неё. Linux по той же причине
/// включает x2APIC сам только при перераспределении прерываний (VT-d), а
/// иначе оставляет режим прошивки.
///
/// Поэтому x2APIC — только если прошивка уже включила его (бит `EXTD` стоит на
/// входе). Такая прошивка к нему готова, а номера APIC шире байта без него не
/// адресуются. Во всех остальных случаях — xAPIC, и это не компромисс по
/// проверенности: процессор QEMU по умолчанию x2APIC не умеет, и стенд ходит
/// этим путём больше сотни прогонов, а x2APIC видели считанные.
fn enable_apic() -> bool {
    ...
    if features.ecx & CPUID_ECX_X2APIC != 0 && base_msr & APIC_BASE_EXTD != 0 {
        // Прошивка передала x2APIC включённым — `EN` и `EXTD` уже стоят, и
        // вернуться в xAPIC без выключения APIC нельзя. Остаётся режим как есть.
        X2APIC.store(true, Ordering::Relaxed);
        kprintln!(
            "  apic        : x2APIC mode (MSR block {X2APIC_MSR_BASE:#x}), left on by the firmware, bsp={is_bsp}"
        );
        return true;
    }
...
/// # Почему PIT первым
///
/// Он адресуется четырьмя фиксированными портами, известен с 1981 года и
/// присутствует на всём, что притворяется PC-совместимым. Ничего искать не
/// нужно: ни таблиц, ни адресов — то есть на пути к измерению нет ни одного
/// места, где можно ошибиться в адресе.
///
/// Но и отказать он может: измерение опирается на **бит 5 порта 0x61**, где
/// видно состояние выхода канала 2, и гипервизор вправе не эмулировать его. Так
/// и вышло на VirtualBox: PIT есть, а выход не меняется — «calibration against
/// the PIT failed», и система остаётся без таймера, то есть без всего, что
/// зависит от времени. Второй источник — [`calibrate_against_pm_timer`].
///
...
/// Сколько длится измерение по таймеру ACPI.
///
/// Десять миллисекунд, как у PIT: при частоте счётчика в десятки мегагерц это
/// сотни тысяч отсчётов, то есть погрешность одного тика ничего не значит, а к
/// загрузке добавляется время, которого не видно.
const PM_CALIBRATION_MS: u64 = 10;

/// Измерить частоту счётчика Local APIC по таймеру ACPI.
///
/// Второй источник существует ровно потому, что первый отказал на живой машине,
/// а не «на всякий случай». Частота таймера ACPI закреплена спецификацией
/// (3,579545 МГц), адрес его порта лежит в FADT, и тем же способом ядро уже
/// измеряет `rdtsc` — поэтому разбор FADT берётся оттуда же
/// ([`super::tsc::PmTimer`]), а не пишется второй раз.
///
/// Счётчик APIC считает **вниз** от начального значения, поэтому наружу отдаётся
/// разность: измеряющей стороне нужен возрастающий счётчик.
fn calibrate_against_pm_timer() -> Option<u64> {
    // SAFETY: адрес RSDP запомнен при разборе хэндоффа и указывает на таблицы
    // прошивки в памяти типа `AcpiReclaimable`, которую ядро не переиспользует;
    // ноль означает «таблиц нет» и обрабатывается внутри.
    let timer = unsafe { super::tsc::PmTimer::find(crate::acpi::rsdp()) }?;

    // Счётчик запускается наибольшим значением: 32 разряда при делителе 16 не
    // успевают обернуться за десять миллисекунд ни на одной мыслимой частоте
    // шины — при 4 ГГц это 2,5 миллиона отсчётов из четырёх миллиардов.
    write_reg(REG_TIMER_INITIAL_COUNT, u32::MAX);
    let measured = timer.measure_against(PM_CALIBRATION_MS, || {
        u64::from(u32::MAX - read_reg(REG_TIMER_CURRENT_COUNT))
    });
    // Остановить счётчик: нулевой начальный счёт — это «таймер выключен».
    write_reg(REG_TIMER_INITIAL_COUNT, 0);
...
fn start_timer() {
    ...
    // Два источника, в порядке «сначала тот, которому ничего не нужно знать».
    // Отказ первого печатается всегда: он означает, что машина ведёт себя не
    // как PC, и это стоит знать до того, как что-нибудь ещё пойдёт не так.
    let (hz, source) = match calibrate() {
        Some(hz) => (hz, "the PIT"),
        None => {
            kprintln!("  timer       : calibration against the PIT failed; trying the ACPI timer");
            match calibrate_against_pm_timer() {
                Some(hz) => (hz, "the ACPI timer"),
                None => {
                    kprintln!("  timer       : the ACPI timer did not help either; timer disabled");
                    kprintln!(
                        "                (a timer running at an unknown rate would make uptime lie)"
                    );
                    return;
                }
            }
