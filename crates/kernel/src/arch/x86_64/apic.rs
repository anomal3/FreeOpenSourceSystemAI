...
/// Первый вектор диапазона, раздаваемого устройствам с MSI.
///
/// Раньше здесь стоял один зашитый вектор для xHCI — единственного устройства,
/// умевшего MSI. Комментарий рядом с той функцией обещал обобщить её, когда
/// появится второе; оно появилось (сетевая карта, фаза 52), и обещание
/// исполнено: векторы раздаёт `interrupts::alloc_msi`, а кто их получил —
/// видно по таблице обработчиков, а не по имени константы.
///
/// Все они приходят по MSI/MSI-X, а не через I/O APIC: устройства лежат на
/// PCIe, где линий прерываний нет вовсе, а есть запись в память.
pub const VECTOR_MSI_FIRST: u8 = 0x40;

/// Сколько устройств могут получить свой вектор.
///
/// Восемь — с запасом на всю фазу 52 (сеть, диски, два контроллера USB) и без
/// расчёта на будущее, которого никто не видел: диапазон расширяется одной
/// цифрой, а таблица обработчиков — массивом того же размера.
pub const MSI_VECTORS: usize = 8;

/// Первый вектор под линии `INTx`.
///
/// Отдельный диапазон, а не общий с MSI, и не ради порядка: линия
/// **разделяемая и уровневая**, и подтверждать её надо иначе, чем MSI. Общий
/// диапазон означал бы проверку «а этот вектор какого рода» в самом горячем
/// месте ядра — в разборе вектора.
pub const VECTOR_INTX_FIRST: u8 = 0x48;

/// Сколько разных линий могут получить вектор. Столько же, сколько ячеек у
/// маршрутизатора в [`crate::irq::routing`]: больше завести всё равно некуда.
pub const INTX_VECTORS: usize = 8;
