...
/// Обработчики векторов MSI: по одному на вектор диапазона, ноль — свободен.
///
/// Атомики с указателем на функцию, а не замок со списком: таблица читается из
/// обработчика прерывания, и замок там — это остановленная машина, если он
/// окажется занят. Записывается она один раз на устройство, при поднятии
/// драйвера, и после этого только читается.
...
extern "C" fn dispatch(frame: *mut TrapFrame) {
    ...
    if vector < VECTOR_FIRST_EXTERNAL && frame.cs & 3 == 3 && crate::user::is_running() {
        ...
        if vector == VECTOR_PAGE_FAULT {
            // Прерывания открываются до первого обращения к файловой системе, и
            // это обязательное условие, а не оптимизация: путь чтения берёт
            // засыпающие мьютексы, а засыпание с закрытым `IF` — это
            // переключение задач, которого никто не ждёт. Прерванный код —
            // третье кольцо, то есть `IF` у него был поднят; возвращаем как
            // было. Кадр лежит на стеке этой задачи (с фазы 41 #PF не на IST),
            // поэтому вытеснение здесь так же безопасно, как в системном
            // вызове, — тот и так исполняется с открытыми прерываниями.
            //
            // Восстанавливается ровно то состояние, в котором исполнялась
            // прерванная программа, и ни одного `SpinLock` этот путь не держит:
            // он пришёл прямо из третьего кольца.
            enable();
    ...
    if vector >= VECTOR_FIRST_EXTERNAL {
        match vector {
            ...
            // MSI от устройства. Обработчик подтверждает прерывание у самого
            // устройства и будит его задачу — разбор колец делает она. Держать
            // разбор здесь было бы можно, но он занимает сотни микросекунд с
            // запрещёнными прерываниями, а прерывания на то и заведены, чтобы
            // их не запрещать.
            vector
                if vector >= apic::VECTOR_MSI_FIRST
                    && usize::from(vector - apic::VECTOR_MSI_FIRST) < apic::MSI_VECTORS =>
            {
                let index = usize::from(vector - apic::VECTOR_MSI_FIRST);
                let handler = MSI_HANDLERS[index].load(Ordering::Acquire);
                if handler != 0 {
                    // SAFETY: в таблице лежат только указатели на `fn()`,
                    // положенные `alloc_msi`; ноль означает пустую ячейку и
                    // отсеян выше. Функция объявлена `extern "Rust" fn()`, то
                    // есть без аргументов и без возвращаемого значения.
                    let handler: fn() = unsafe { core::mem::transmute(handler) };
                    handler();
                } else {
                    // Вектор из диапазона без обработчика — не «не бывает», а
                    // «устройство пишет туда, откуда драйвер уже ушёл».
                    // Подтвердить всё равно обязательно: без EOI APIC не
                    // доставит ничего следующего.
                    kprintln!("interrupts: MSI vector {vector:#04x} has no handler");
                }
                apic::eoi();
            }
            ...
            // Соседний процессор просит остановиться: паника, отказ или
            // выключение. Подтверждать нечего — процессор сюда не вернётся.
            apic::VECTOR_IPI_STOP => crate::smp::on_stop(),
...
fn load_idt() {
    ...
    for vector in 0..IDT_ENTRIES {
        ...
        // Отдельный стек остался только у двойного отказа. #PF его лишился в
        // фазе 41 и не по недосмотру: обработчик научился возвращаться, а
        // возвратный обработчик обязан исполняться на стеке **своей** задачи —
        // иначе вложенный отказ затрёт кадр внешнего (IST даёт один и тот же
        // RSP), а засыпание на общем стеке уведёт с него другую задачу.
        //
        // Цена названа и уплачена в другом месте: переполнение стека ядра
        // теперь приезжает двойным отказом, и распознаёт его `irq::fault` по
        // указателю стека, а не по адресу обращения.
...
/// Загрузить ту же IDT на проснувшемся процессоре.
///
/// Только `lidt`, без заполнения: таблица одна на все процессоры и заполнена
/// загрузочным. Переписывать её здесь значило бы править записи, по которым
/// загрузочный прямо сейчас доставляет прерывания.
pub fn load_idt_secondary() {
    let pointer = DescriptorTablePointer {
        limit: (size_of::<Idt>() - 1) as u16,
        base: IDT.get() as u64,
    };
    // SAFETY: таблица заполнена целиком при запуске загрузочного процессора и
    // живёт всё время работы ядра; её записи ссылаются на селектор кода из общей
    // GDT, которую этот процессор уже загрузил.
    unsafe {
        asm!("lidt [{}]", in(reg) &raw const pointer, options(readonly, nostack, preserves_flags));
    }
}
...
/// Выделить устройству вектор MSI и сказать, куда ему писать.
///
/// Возвращает пару «адрес, данные» для таблицы MSI-X устройства; `None` —
/// свободных векторов не осталось, и драйвер обязан остаться на опросе, а не
/// отказываться работать.
///
/// На x86-64 готовить почти нечего: все векторы уже стоят в IDT — таблица
/// заполняется целиком при загрузке, — и маршрут задаёт само устройство.
/// Работа здесь одна: запомнить, кого звать. Функция существует ещё и ради
/// второй архитектуры, где всё наоборот, и ради того, чтобы драйвер не
/// содержал ни одного `cfg`.
#[must_use]
pub fn alloc_msi(handler: fn()) -> Option<(u64, u32)> {
    for (index, slot) in MSI_HANDLERS.iter().enumerate() {
        // Обмен, а не проверка с последующей записью: два драйвера могут
        // подниматься одновременно, и «свободен» между чтением и записью
        // успело бы стать неправдой.
        if slot
            .compare_exchange(0, handler as usize, Ordering::AcqRel, Ordering::Acquire)
            .is_ok()
        {
            let vector = apic::VECTOR_MSI_FIRST + index as u8;
            return Some(apic::msi_target(vector));
        }
    }
    None
}
