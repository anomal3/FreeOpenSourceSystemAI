...
extern "C" fn dispatch(frame: *mut TrapFrame) {
    ...
    if vector >= VECTOR_FIRST_EXTERNAL {
        ...
        // Вытеснение — здесь и только здесь. EOI уже отправлен всеми ветками
        // выше, то есть локальный APIC снял бит ISR и снова пропускает
        // прерывания этого приоритета; уйди мы в другую задачу раньше, она
        // работала бы с закрытым таймером и вытеснить её было бы нечем.
        //
