...
pub fn init() {
    ...
    XCR0.store(wanted, Ordering::Relaxed);
...
/// Набор компонентов, который [`init`] включил на загрузочном процессоре.
static XCR0: core::sync::atomic::AtomicU64 = core::sync::atomic::AtomicU64::new(0);

/// Включить векторные расширения на проснувшемся процессоре.
///
/// `CR0` и `CR4` он уже получил от трамплина готовыми значениями загрузочного.
/// `XCR0` трамплину недоступен — это не управляющий регистр, а расширенный, и
/// пишется отдельной инструкцией, — поэтому ставится здесь. Набор тот же, что у
/// загрузочного, и это не удобство: область состояния задачи размечена под его
/// размер, а задача, переехавшая на процессор с другим `XCR0`, записала бы
/// `xsave` за конец своей области.
pub fn init_secondary() {
    if !HAVE_XSAVE.load(Ordering::Relaxed) {
        return;
    }
    // SAFETY: `CR4.OSXSAVE` скопирован трамплином, а маска — та, что процессор
    // этой же модели уже принял на загрузочном.
    unsafe { write_xcr0(XCR0.load(Ordering::Relaxed)) };
}
