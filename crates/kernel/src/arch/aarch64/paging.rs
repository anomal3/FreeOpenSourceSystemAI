...
/// `SPAN` = 0 процессор взводит `PSTATE.PAN` **сам**, на входе в каждое
/// исключение. Ядро же кладёт аргументы прямо на стек программы и читает её
/// буферы обычным `memcpy` — и первое же такое обращение кончается отказом прав
/// уровня 3 (`ESR_EL1` = `0x9600000F`), то есть система умирает через полсекунды
/// после запуска `/bin/init`.
///
/// Бит выставляется **безусловно**, без оглядки на `ID_AA64MMFR1_EL1.PAN`: на
/// процессоре без `FEAT_PAN` он RES1, а спрашивать разрешения не у кого —
/// VirtualBox на Apple Silicon объявляет PAN нереализованным и при этом взводит
/// его на входе в исключение. Стенд этого не ловил: QEMU `virt` PAN не
/// реализует вовсе.
///
/// Правильная защита — держать PAN взведённым и снимать лишь на время
/// проверенного обращения к памяти пользователя (как `uaccess` в Linux). Это
...
/// Снять отображение одной страницы и вернуть кадр, который под ней лежал.
///
/// `None` означает «страница и так не отображена» — то есть работа уже сделана,
/// а не «сделать не удалось». Различать эти два случая здесь не нужно: снимает
/// отображение только тот, кто его и ставил, и повторный вызов на уже снятой
/// странице — не ошибка, а гонка, которой всё равно нечего вернуть в пул.
///
/// Промежуточные таблицы **остаются на месте**, даже опустев. Освобождать их
/// значило бы считать в каждой число живых записей — счётчик на таблицу, четыре
/// уровня, и вся эта арифметика ради нескольких страниц: пустая таблица стоит
/// одну, а держится их у программы единицы. Всё это возвращает разбор
/// пространства при выходе, обходя поддерево целиком (`free_user_space`), и
/// пустая таблица ему не мешает — он не смотрит, чем она была занята.
///
/// # Safety
///
/// `root` обязан быть корнем той половины, которой принадлежит `virt`, и быть
/// доступен через действующее прямое отображение; кадр под `virt` — принадлежать
...
pub unsafe fn unmap(root: PhysAddr, virt: VirtAddr) -> Option<(PhysAddr, usize)> {
    ...
    let slot = table_entry(table, virt.table_index(LEAF_LEVEL));
    // SAFETY: `slot` — запись таблицы L3, полученной спуском выше.
    let leaf = unsafe { ptr::read_volatile(slot) };
    if leaf & DESC_VALID == 0 {
        return None;
    }

    // SAFETY: тот же слот той же таблицы. Дескриптор обнуляется целиком: гасить
    // один бит валидности значило бы оставить в таблице адрес кадра, который
    // сейчас уедет в чужие руки.
    unsafe { ptr::write_volatile(slot, 0) };
    // Обнуление обязано стать видно table walker'у прежде, чем мы попросим его
    // забыть страницу: `tlbi` по недописанной таблице перечитал бы прежний
    // дескриптор и завёл трансляцию заново.
    dsb_ishst();

    // Порядок обязателен: пока трансляция сидит в TLB, процессор ходит по
    // старому кадру, не спрашивая таблицу. Отдать такой кадр в пул до сброса
    // означает отдать его вместе с работающей дорогой к нему. Сброс — по всем
    // ядрам (`vaae1is`), потому что дешевле разослать его сейчас, чем помнить,
    // на каком ядре эта программа успела поработать.
...
pub unsafe fn activate_space(root: PhysAddr) {
    ...
    //
    // Сброс **локальный**, без `is`, и с фазы 43 это разница, а не деталь. `is`
    // рассылает сброс всем процессорам машины; пока процессор был один, это
    // ничего не стоило. Теперь каждое переключение задач на любом процессоре
    // останавливало бы остальные до подтверждения сброса — а эмулятор для этого
    // и вовсе останавливает мир целиком. Нужды в рассылке нет: трансляции
    // покидаемого пространства живут только в TLB того процессора, который его
    // покидает (задача исполняется на одном процессоре за раз), и сбрасывает
    // их именно он — здесь. См. заголовок `crate::smp`.
    unsafe {
        asm!(
            "dsb ishst",
            "msr ttbr0_el1, {root}",
