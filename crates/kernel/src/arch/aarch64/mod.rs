...
/// Поставить базу хранилища потока — `TPIDR_EL0`.
///
/// # Ловушка на одну букву
///
/// `TPIDR_EL1` **занят**: в нём лежит номер процессора (см. `smp`), и читает
/// его всё, что спрашивает «на каком я ядре». Потоку принадлежит регистр
/// третьего кольца, `TPIDR_EL0`. Перепутать их — значит менять номер
/// процессора на каждом переключении задач; SMP при этом не упадёт сразу, а
/// начнёт путать процессоры.
pub fn set_user_tls(base: u64) {
    // SAFETY: запись в `TPIDR_EL0` не меняет ничего, кроме значения, которое
    // читает третье кольцо. Ядро этим регистром не пользуется.
    unsafe {
        asm!("msr tpidr_el0, {}", in(reg) base, options(nomem, nostack, preserves_flags));
    }
}
...
/// После паники перезагрузка по клавише — только у x86-64, где клавиатуру PS/2
/// можно опросить без прерываний. Здесь такой клавиатуры нет: остаётся сброс.
///
/// # Safety
///
/// Ничего не делает; `unsafe` ради одного договора на обе архитектуры.
pub unsafe fn reboot_on_key(_rsdp: u64) -> bool {
    false
}
