...
aarch64_enter_user:
    ...
    // Задача пришла сюда с открытыми прерываниями, а ELR/SPSR ниже — это
    // регистры, которые прерывание при входе перезаписывает своей точкой
    // возврата. Пришедшее между `msr elr_el1` и `eret`, оно отправило бы
    // программу по адресу ядра (тот же дефект, что в заглушке ловушки, см.
    // `aarch64_trap_common`). Закрываем всё; маска EL0 придёт из SPSR при
    // `eret`, и в программе прерывания будут открыты.
    msr     daifset, #0xf
