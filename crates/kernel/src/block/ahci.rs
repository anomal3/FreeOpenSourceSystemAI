...
//!
//! # Как ожидается завершение
//!
//! Прерыванием, если контроллер согласился его прислать (фаза 52), и холостым
//! циклом, если нет. Прерывание тут не ради скорости — команда от этого не
//! выполняется быстрее, — а ради того, чем занят процессор, пока диск отвечает:
//! на настоящем SATA ответ идёт миллисекунды, и всё это время крутить `PxCI` в
//! цикле означает держать ядро занятым ничем.
//!
//! Холостой цикл остаётся не как пережиток, а как обязательный путь: корень
//! монтируется **до** запуска планировщика, и спать там некому и не на чем. По
//! той же причине сон ограничен коротким сроком и повторяется: потерянное
//! прерывание стоит один срок, а не остановившееся ядро.
//!
//! Ограничение по времени на команду при этом настоящее на обоих путях, по
//! монотонному счётчику из фазы 17: отказавший диск обязан привести к
//! сообщению, а не к молча остановившемуся ядру.
//!
...
/// Какие порты просят внимания. Бит на порт; снимается записью единицы.
...
/// `GHC.IE` — контроллеру разрешено прерывать процессор. Общий выключатель над
/// всеми `PxIE`: при сброшенном бите порт может сколько угодно взводить `PxIS`,
/// наружу это не выйдет.
...
/// Какие события порта нам интересны: пришёл кадр регистров от устройства
/// (`DHRS`) — обычное завершение команды DMA, кадр PIO (`PSS`), кадр данных
/// (`DSS`), кадр очереди (`SDBS`) и ошибка (`TFES`).
///
/// Ошибка включена не для полноты: при отказе устройство вправе не снять бит
/// слота, и без её прерывания ждущий проснулся бы только по сроку — то есть
/// через десять секунд вместо сразу.
const PORT_INTERRUPTS: u32 = (1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | IS_TFES;
...
/// На сколько засыпать за раз, ожидая завершения. Как у virtio-blk, и по той же
/// причине: это срок **неполучения** прерывания, а не срок ответа диска.
const SLEEP_SLICE_MS: u64 = 1;

/// Ярлык, по которому планировщик будит ждущих ответа от AHCI.
///
/// Он один на весь контроллер, а не на порт, потому что прерывание одно на весь
/// контроллер. Разбудить лишнего ждущего не вредно: проснувшись, каждый
/// проверяет **свой** `PxCI`, и чужое завершение он просто не увидит.
const IRQ_SOURCE: u32 = crate::irq::source::AHCI;

/// Окно регистров контроллера — для обработчика прерывания.
///
/// Обработчик вызывается без аргументов, а снять признак прерывания нужно
/// именно в регистрах: адрес приходится оставлять здесь. Ноль означает «MSI не
/// настроен», и это же начальное значение. Контроллер поддерживается один — тот
/// же, что и в `probe`.
static CONTROLLER: core::sync::atomic::AtomicUsize = core::sync::atomic::AtomicUsize::new(0);

/// Прерывание от контроллера AHCI.
///
/// Признак снимается здесь и в правильном порядке — сначала у порта, потом у
/// контроллера. Обратный порядок оставил бы бит порта взведённым, и следующий
/// же взгляд контроллера снова поднял бы прерывание: MSI от этого не зациклится
/// (он не уровневый), но `HBA_IS` больше никогда не стал бы нулём.
pub fn on_interrupt() {
    let base = CONTROLLER.load(core::sync::atomic::Ordering::Relaxed);
    if base != 0 {
        let base = VirtAddr::new(base);
        // SAFETY: адрес положен сюда только после отображения окна, и окно
        // живёт всё время работы ядра.
        unsafe {
            let pending = read32(base, HBA_IS);
            for index in 0..MAX_PORTS {
                if pending & (1 << index) == 0 {
                    continue;
                }
                let port = VirtAddr::new(base.as_usize() + PORT_BASE + index * PORT_STRIDE);
                // Читается и записывается обратно то же значение: единица
                // снимает бит, ноль его не трогает. Записывать `u32::MAX` здесь
                // нельзя — так снялись бы и признаки, взведённые между чтением
                // и записью, то есть события, о которых никто не узнает.
                let status = read32(port, PX_IS);
                write32(port, PX_IS, status);
            }
            write32(base, HBA_IS, pending);
        }
    }
...
unsafe fn attach(device: &Device, disks: &mut Vec<AhciDisk>) -> Result<(), AhciError> {
    ...
    // Прерывание поднимается до запуска портов: `PxIE` пишется в `start`, и к
    // этому моменту уже должно быть известно, писать туда что-то или ноль.
    // SAFETY: окно отображено, режим AHCI включён выше.
    let irq = unsafe { enable_interrupts(device, base) };
    ...
    for index in 0..MAX_PORTS {
        ...
        // SAFETY: контракт функции.
        match unsafe { AhciDisk::start(port, index, irq) } {
            Ok(disk) => {
                kprintln!(
...
/// Попросить контроллер присылать прерывания и подготовить их приём.
///
/// Возвращает ярлык для планировщика или `None`, если что-то из цепочки
/// недоступно: у контроллера нет MSI, у машины нет цели для него (GICv3 без
/// v2m), — тогда драйвер остаётся на холостом цикле и работает ровно как
/// раньше. Отказ здесь не ошибка и не повод не поднимать диск.
///
/// # Safety
///
/// `base` должен указывать на отображённое окно регистров контроллера.
unsafe fn enable_interrupts(device: &Device, base: VirtAddr) -> Option<u32> {
    let Some(msi) = device.msi() else {
        kprintln!("  ahci        : no MSI capability; completions will be spun on");
        return None;
    };
    let Some((address, data)) = crate::arch::interrupts::alloc_msi(on_interrupt) else {
        kprintln!("  ahci        : no MSI target on this machine; completions will be spun on");
        return None;
    };

    // Адрес окна кладётся **до** разрешения доставки: прерывание может прийти
    // сразу после записи в `Message Control`, а обработчик без адреса не снимет
    // признак у порта.
    CONTROLLER.store(base.as_usize(), core::sync::atomic::Ordering::Relaxed);

    // SAFETY: контракт функции; обработчик поставлен `alloc_msi` выше.
    unsafe {
        // Признаки, накопленные прошивкой, снимаются до разрешения: иначе
        // первое же прерывание придёт за событие, которого мы не видели.
        write32(base, HBA_IS, u32::MAX);
        device.set_msi_vector(&msi, address, data);
        let ghc = read32(base, HBA_GHC);
        write32(base, HBA_GHC, ghc | GHC_IE);
    }

    kprintln!(
        "  ahci        : MSI -> {address:#018x} data {data:#x}, completions arrive by interrupt"
    );
    Some(IRQ_SOURCE)
}
...
impl AhciDisk {
    ...
    unsafe fn start(port: VirtAddr, port_index: usize, irq: Option<u32>) -> Result<Self, AhciError> {
        // SAFETY: контракт функции.
        unsafe { stop(port) }?;
        ...
        unsafe {
            ...
            // События порта разрешаются, только если прерывание действительно
            // настроено. Разрешить их без обработчика — верный способ получить
            // бесконечный уровневый прерыватель на общей линии.
            write32(port, PX_IE, if irq.is_some() { PORT_INTERRUPTS } else { 0 });
        }
    ...
    ///
    /// Ждать можно двумя способами, и выбор делается один раз на команду, а не
    /// на каждом обороте: либо сном до прерывания, либо холостым циклом. Второй
    /// нужен не только машинам без MSI — корень монтируется до запуска
    /// планировщика, и до этого момента засыпать не на чем.
    ///
    /// # Safety
    ///
    ...
    unsafe fn wait_for_completion(&self) -> Result<(), AhciError> {
        ...
        let sleep = self.irq.filter(|_| crate::sched::is_running());
        loop {
            // SAFETY: контракт функции.
            ...
            match sleep {
                Some(source) => {
                    let port = self.port;
                    // Условие проверяется ещё раз и уже под локом планировщика:
                    // между проверкой выше и засыпанием прерывание могло прийти,
                    // и без этой проверки задача уснула бы после того, как её
                    // разбудили. Читать регистры так безопасно — чтение ничего
                    // не снимает, в отличие от кольца завершений virtio.
                    crate::sched::block_on_irq_until(source, SLEEP_SLICE_MS, || {
                        // SAFETY: окно порта отображено всё время жизни диска.
                        let (ci, is, tfd) = unsafe {
                            (read32(port, PX_CI), read32(port, PX_IS), read32(port, PX_TFD))
                        };
                        ci & 1 == 0 || is & IS_TFES != 0 || tfd & TFD_ERR != 0
                    });
                }
