...
fn announce() {
    ...
    // Здесь же — отметка о том, что человек за машиной есть. Место выбрано
    // единственное на всю систему нарочно: через `announce` проходит и клавиша,
    // и мышь, и палец, а отметка, расставленная по драйверам, однажды забылась
    // бы в новом.
    crate::power::note_input();
