...
/// Разбирает ли кольцо прямо сейчас какой-нибудь процессор.
///
/// # Зачем признак, если кольцо и так под локом
///
/// Затем, что лок кольца отпускается **до** разбора байта — иначе разбор (а он
/// доходит до планировщика) шёл бы из-под лока кольца, и порядок захвата
/// «кольцо → планировщик» встретился бы с существующим «планировщик → serial →
/// кольцо». Но в этот промежуток второй процессор успевает вынуть следующий
/// байт и скормить его декодеру **раньше** первого.
///
/// Стоило это команды `run -b /bin/spin 6000`, приехавшей в оболочку как
/// `n r-b u/bin/spin 6000`: байты не потерялись, они переставились местами.
/// Ловится только на четырёх процессорах и только в release — там промежуток
/// между `pop` и `feed` достаточно узок, чтобы в него попал соседний процессор,
/// и достаточно широк, чтобы это случалось.
///
/// Правило «два процессора, читающие одно FIFO, переставили бы символы команды»
/// было записано для чтения железа и выполнено там. Здесь оно же нарушалось на
/// один уровень выше, между кольцом и декодером.
...
/// удерживается ни одного лока.
///
/// Разбирает **один** процессор за раз (см. [`DISPATCHING`]), но лок кольца при
/// этом не удерживается: спасение из печати на соседнем процессоре продолжает
/// складывать байты, пока мы их разбираем, — а именно ради этого кольцо и
/// заведено.
pub fn dispatch() {
    loop {
        // Не смогли взять признак — значит разбором занят другой процессор, и
        // наши байты достанутся ему: он крутится, пока кольцо не опустеет.
        if DISPATCHING
        ...
        // Между последним `pop` и снятием признака сосед мог положить байт и
        // уйти, решив, что разберём мы. Проверка закрывает это окно: без неё
        // последний байт команды ждал бы следующего прерывания — то есть
        // тика таймера, и выглядело бы это как «оболочка не видит Enter».
        let empty = PENDING.try_lock().is_none_or(|pending| pending.len == 0);
        if empty {
            return;
        }
    }
