...
//! и это осознанная замена: соединений у нас десятки, порты эфемерные выдаются
//! по кругу из шестнадцати тысяч, а держать слот занятым минуту при
//! [`MAX_STREAMS`] слотах означало бы, что веб-сервер, ответивший на десяток
//! запросов подряд, исчерпывает систему.
...
///
/// Было восемь, стало шестнадцать — с появлением веб-сервера (`/bin/httpd`).
/// Восьми ему не хватает арифметически, а не «впритык»: слушающий сокет,
/// четыре клиентских и по одному соединению к тому, кого он проксирует, на
/// каждого клиента — это девять, и это только один сервер. Прежний предел
/// означал бы, что второй запрос к прокси получает отказ, и выглядел бы он у
/// клиента как неисправная сеть.
///
/// Цена — только память, и только занятых слотов: буферы соединения
/// ([`RECEIVE_BUFFER`] и [`SEND_BUFFER`]) — это `VecDeque`, которые ничего не
/// занимают, пока в соединение не пришёл первый байт. Пустой слот стоит
/// размера `Option<Connection>`.
