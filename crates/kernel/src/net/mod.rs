...
const IRQ_SOURCE: u32 = crate::irq::source::NET;
...
pub fn stream_recv(owner: TaskId, index: usize, out: &mut [u8]) -> Result<usize, NetError> {
    ...
    let (taken, need_window_update) = {
        ...
        // Окно **до** чтения: по нему решается, узнал ли собеседник, что место
        // освободилось.
        ...
        // Освободившееся место надо объявить: собеседник, которому мы сказали
        // «окно почти ноль», сам не догадается, что оно открылось, и будет
        // молчать до своего таймера настойчивости — а это секунды.
        //
        // Порог — целый сегмент (или половина буфера, если он меньше). Это
        // правило приёмной стороны из RFC 1122 §4.2.3.3 против «синдрома
        // глупого окна»: объявлять по байту значит звать собеседника прислать
        // байт, а молчать до полного нуля — значит останавливать поток на
        // каждом буфере.
        //
        // Дефект, который эта строка закрывает, стоил фазы 39. Условие было
        // `окно == 0`, и на потоковой загрузке оно почти никогда не выполнялось:
        // программа успевала прочитать раньше, чем последний сегмент добивал
        // окно до нуля, — обновление не уходило, отправитель ждал свой таймер по
        // пять секунд, и семьдесят семь мегабайт качались со скоростью около
        // трёх килобайт в секунду. На коротких обменах (SSH, эхо) этого не видно
        // вовсе: там окно не кончается.
        let threshold = usize::from(conn.peer_mss).max(536).min(stream::RECEIVE_BUFFER / 2);
        let after = conn.window();
        (
            taken,
            taken > 0 && usize::from(before) < threshold && usize::from(after) >= threshold,
        )
    };
    ...
    for index in indices {
        ...
        if let Some(conn) = iface.streams.at(index) {
            if conn.timed_out(now) {
                if conn.in_flight() == 0 {
                    // Таймер сработал, а повторять нечего: всё, что мы
                    // отправляли, подтверждено. Так живёт **скачивающий** —
                    // запрос ушёл и подтверждён вместе с первым куском ответа, а
                    // дальше от нас идут одни подтверждения.
                    //
                    // Дефект, который эта ветка закрывает, стоил фазы 39: без
                    // неё каждое такое срабатывание считалось неудачей, шесть
                    // подряд объявляли соединение мёртвым — и загрузка образа
                    // обрывалась на исправной связи через полминуты, «reset»
                    // приходил от **нас**, а выглядело это как обрыв у сервера.
