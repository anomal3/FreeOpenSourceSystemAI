...
//!
//! Свою работу она уже сделала: строка «`ohci` … no driver here», снятая с
//! настоящей машины, и назвала контроллер, под который написан второй драйвер
//! ([`super::ohci`]). Приговор в этой таблице после этого изменился — и меняться
//! он обязан здесь же, вместе с драйверами: перепись, которая врёт про
//! собственные умения, хуже её отсутствия.
//!
...
const fn describe(prog_if: u8) -> (&'static str, bool) {
    match prog_if {
        ...
        // OHCI ядро умеет с фазы 38a — драйвер написан ровно по той строке,
        // которую печатает эта перепись на машине с VirtualBox.
        PROG_IF_OHCI => ("ohci", true),
        // EHCI ядро умеет с тех пор, как эта перепись назвала его на ноутбуке с
        // Intel Cougar Point: спутника там нет, а клавиатура и мышь висят на
        // встроенном хабе за ним (см. [`super::ehci`]).
        PROG_IF_EHCI => ("ehci", true),
