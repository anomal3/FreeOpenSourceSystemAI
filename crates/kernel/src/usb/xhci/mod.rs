...
/// Как часто опрашивать контроллер.
///
/// Прерываний xHCI ядро не использует (см. заголовок модуля), поэтому отчёты
/// устройств забирает опрос. Десять миллисекунд — это период опроса конечной
/// точки прерываний у обеих загрузочных устройств: чаще бессмысленно, реже —
/// заметно пальцам.
const POLL_PERIOD_MS: u64 = 10;

/// Тело задачи, опрашивающей контроллер.
///
...
/// ввода, оказалось, что ждать ей нечего: события ввода рождает этот самый
/// опрос, и оболочка, уснувшая до него, не проснулась бы никогда. Круг
/// разрывается тем, что опрос стал отдельной задачей со своими часами.
pub fn poll_task() {
    loop {
        service();
