...
//!
//! EHCI появился третьим ([`ehci`]), и тоже по переписи с настоящей машины: на
//! чипсетах Intel начиная с Cougar Point спутников у него нет, а клавиатура и
//! мышь висят на встроенном хабе за ним. UHCI по-прежнему пропущен, и это
//! названо вслух.
//!
...
pub fn find_hid(bytes: &[u8]) -> Option<HidInterface> {
    find_hid_nth(bytes, 0)
}

/// Найти `skip`-й по счёту интерфейс HID с конечной точкой прерываний.
///
/// # Зачем считать, а не брать первый
///
/// Потому что составное устройство — это норма, а не редкость. Приёмник
/// беспроводной мыши почти всегда объявляет два-три интерфейса: нулевой —
/// клавиатура (чтобы работать в меню загрузки), первый — мышь, иногда второй —
/// собственные кнопки изготовителя. Драйвер, берущий первый интерфейс, поднимает
/// у такого приёмника клавиатуру, которая никогда ничего не пришлёт, и не
/// открывает точку, по которой едет мышь.
///
/// Стоило это ровно одной ночи: на ноутбуке Романа USB-клавиатура заработала
/// сразу, а беспроводная мышь не подавала признаков жизни и не значилась ни в
/// одном журнале — потому что её интерфейс никто не спрашивал. На QEMU этого не
/// видно вовсе: там клавиатура и мышь всегда отдельные устройства с одним
/// интерфейсом каждое.
#[must_use]
pub fn find_hid_nth(bytes: &[u8], skip: u8) -> Option<HidInterface> {
            ...
            DESC_ENDPOINT if length >= 7 => {
                if let Some(mut found) = current {
                    ...
                    if attributes & 0b11 == 0b11 && address & REQ_DIR_IN != 0 {
                        ...
                        // Этот интерфейс посчитан — вторая его точка (у
                        // клавиатуры это светодиоды) не должна сойти за
                        // следующий интерфейс.
                        seen += 1;
                        current = None;
                    }
