...

    {
        ...
        if sched.running {
            ...
            if let Some(next) = sched.pick_next() {
                ...
                // Векторные регистры — здесь же, под тем же локом и с теми же
                // запрещёнными прерываниями. Порядок тот же, что у машинного
                // состояния программы: сохранить своё, поставить чужое.
                //
                // Между этими двумя строками и `switch_context` ниже ядро
                // векторов не трогает — оно собрано без них на обеих
                // архитектурах, — поэтому переставлять их до смены стека
                // безопасно и дешевле: не надо второй раз брать лок.
                //
                // У задачи может не быть области (холостая, а также любая, для
                // которой не хватило кучи). Тогда её регистры не сохраняются и
                // не восстанавливаются — и это верно ровно потому, что такая
                // задача их не трогает: программы без области не бывает.
                if !from.is_null() && !to.is_null() {
                    ...
                    if let Some(area) = save_to {
                        // SAFETY: область принадлежит задаче, которая исполняется
                        // прямо сейчас, выделена с нужным размером и
                        // выравниванием и жива, пока жива задача.
                        unsafe { arch::fpu::save(area) };
                    }
                    if let Some(area) = load_from {
                        // SAFETY: область заполнена либо `init_area` при
                        // создании задачи, либо предыдущим сохранением на этой
                        // же машине.
...
pub fn preempt_point() {
    ...
    // Кто-то просил не отбирать процессор — например, задача печатает строку в
    // журнал. Флаг `NEED_RESCHED` при этом **не сбрасывается**: он подождёт
    // ближайшего прерывания после того, как просьбу снимут, и квант окажется
    // длиннее заказанного на время просьбы, а не потерянным.
...
/// человек, и стенд. Собирается она из кусков (`write_fmt` пишет отдельно текст
/// и отдельно каждую подстановку), и с вытеснением её разрывает чужой вывод
/// посередине:
///
/// ```text
///   /bin/vec: started as   user        : #10 '/bin/sshd' as uid 0 ...
/// ```
///
/// Лока вывода для этого мало, и в этом суть дефекта: писателей два —
/// [`crate::shell::print`] берёт свой лок, а `kprintln!` о нём не знает вовсе.
/// Договориться им не о чем: второй печатает из мест, где ждать нельзя (в том
/// числе из обработчика отказа). Единственное, что делает строку целой на
/// одноядерной машине, — не дать никому другому выполниться, пока она пишется.
///
/// Прерывания при этом **не** запрещаются: строка в UART уходит байтами с
/// ожиданием готовности, и запрет прерываний на всё это время означал бы
/// потерянные тики и просроченные пакеты. Обработчик прерывания, который сам
/// что-то печатает, строку по-прежнему разорвёт — но он печатает только тогда,
/// когда случилось что-то, о чём важнее сообщить.
...
///
/// # Зачем отдельный счётчик, а не общий
///
/// Затем, что приглашение ядра ждёт **их**, а не «всех остальных». Раньше оно
/// ждало, пока не останется единственной живой задачей, — и это работало ровно
/// до тех пор, пока обычной задачей был только оно само да демонстрация.
///
/// На телефоне условие стало невыполнимым навсегда: задача USB заведена
/// обычной намеренно (пока кабель — единственный вход в машину, обмен по нему и
/// есть работа, ради которой она включена), счётчик живых никогда не опускается
/// до единицы, и приглашение остаётся в ожидании. Снаружи это выглядит так:
/// стол нарисован, часы на нём стоят, курсор не двигается — при том что
...
pub fn spawn_demo_tasks() -> usize {
    ...
    for name in ["alpha", "beta", "gamma"] {
        ...
        // что говорить уже некому, и вклинивается в чужой вывод.
