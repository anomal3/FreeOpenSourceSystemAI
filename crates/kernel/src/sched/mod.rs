...
/// Длина кванта в тиках таймера: 50 мс — заметно больше, чем стоит
/// переключение, и заметно меньше, чем человек замечает.
///
/// Считается в тиках, а тик стал вдесятеро мельче (см. [`crate::irq::TIMER_HZ`]),
/// поэтому число выросло с пяти до пятидесяти. Квант при этом прежний: менять
/// его заодно с частотой таймера значило бы поменять две вещи разом и не
/// узнать, которая из них подействовала.
const SLICE_TICKS: u32 = 50;
