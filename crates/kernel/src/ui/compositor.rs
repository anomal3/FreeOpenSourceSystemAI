...
impl Compositor {
    ...
    pub fn drag_by(&mut self, dx: i32, dy: i32) {
        // На телефоне окна не двигаются и не меняют размера: они занимают
        // экран целиком, и переносить их некуда.
        //
        // Это не только про смысл, но и про цену, и цена измерена. Каждый сдвиг
        // помечает **два** прямоугольника размером с окно — откуда ушло и куда
        // пришло, — то есть два экрана. Буфер кадра держит 177 строк, значит
        // каждый такой прямоугольник режется на тринадцать полос, и в каждой
        // полосе заново рисуются все слои. В замере это выглядело как
        // подорожавшая отрисовка (обои 49 мс, значки 32 мс, окна 77 мс), хотя
        // отрисовка та же — просто повторённая тринадцать раз на кадр.
        if theme::is_mobile() {
            return;
        }
