...
fn log_only(parser: &Parser, final_byte: u8) {
    ...
    match final_byte {
        ...
        // Цвет (`m`) не называется. Лимит строк журнала общий, а приглашение
        // оболочки с тех пор, как оно зелёное, шлёт по два цвета на каждую
        // команду: через полтора десятка команд лимит был бы съеден, и
        // программа, ради которой журнал и заведён (стирание экрана `mc`),
        // осталась бы без единой строки. Цвет виден на снимке, раскладка — нет.
        b'J' | b'K' => log(format_args!(
            "CSI {}{}",
