...
pub enum Action {
    ...
    ClockSettings,
    /// Диспетчер задач — программа `/bin/taskmgr` (фаза С5). До неё пункт
    /// стоял с пометкой «функция запланирована»: правило дома — нереализованное
...
impl Action {
    ...
    fn title(self) -> &'static str {
        match self {
            ...
            Action::TaskManager => "Диспетчер задач",
        }
    }
