...
/// На сколько засыпать за раз, ожидая завершения от устройства.
///
/// Миллисекунда — не срок ответа диска, а срок **неполучения прерывания**:
/// нормальный ответ приходит прерыванием и будит раньше. Меньше ставить
/// незачем, больше — значит платить этим сроком за каждое потерянное
/// прерывание.
const SLEEP_SLICE_MS: u64 = 1;

/// Наименьшее число засыпаний, даже если предел опроса мал.
///
/// Предел опроса задан в оборотах холостого цикла, а здесь считаются
/// миллисекунды; перевод между ними — это оценка скорости процессора, которой у
/// нас нет. Нижняя граница избавляет от неё: сколько бы ни было оборотов,
/// подождать стоит хотя бы столько.
const POLL_SLEEP_ATTEMPTS_MIN: u32 = 200;
...
impl Queue {
    ...
    /// Есть ли завершение, не забирая его.
    ///
    /// Нужна проверке «не пришло ли уже», которую планировщик зовёт **под своим
    /// локом**: забирать там завершение нельзя — засыпающий может и не уснуть,
    /// и тогда забранное исчезло бы, не дойдя до того, кто его ждёт.
    ...
    /// зависшим. Предел стоит на **обоих** путях ожидания и означает одно и то
    /// же: «устройство не ответило». Разница между путями — чем занят
    /// процессор, пока диск отвечает.
    ///
    /// `irq` — ярлык прерывания этой очереди, если устройство согласилось его
    /// присылать. `None` — ждём холостым циклом, как ядро ждало всегда.
    fn submit_and_wait(&mut self, polls: u32, irq: Option<u32>) -> Result<(), VirtioError> {
        self.offer(0);
        self.kick();

        // Сон возможен не всегда, и это не мелочь: корень монтируется **до**
        // запуска планировщика, и спать там некому и не на чем. Проверка одна
        // и здесь: ниже по коду обе ветви уже про то, как ждать, а не про то,
        // можно ли.
        if let Some(source) = irq.filter(|_| crate::sched::is_running()) {
            return self.sleep_until_used(polls, source);
        }
    ...
    /// Дождаться завершения сном.
    ///
    /// Срок у каждого сна короткий и намеренно: прерывание, потерянное по любой
    /// причине — устройство не прислало, вектор не дошёл, — не превращается в
    /// задачу, уснувшую навсегда. Цена потери при этом ровно один срок, а не
    /// зависшая машина; та же осторожность, что у `Wait::Input`.
    fn sleep_until_used(&mut self, polls: u32, source: u32) -> Result<(), VirtioError> {
        // Столько же ожиданий, сколько было бы оборотов опроса, но каждое — сон
        // на миллисекунду вместо холостого цикла. Предел терпения при этом тот
        // же по смыслу, а не по числу тактов: «устройство молчит слишком долго».
        let attempts = (polls / 1000).max(POLL_SLEEP_ATTEMPTS_MIN);
        for _ in 0..attempts {
            if self.take_used().is_some() {
                return Ok(());
            }
            // Проверка «не пришло ли уже» идёт под локом планировщика, и она
            // обязана быть **невзятием**: завершение, забранное здесь, пропало
            // бы, если засыпающий после неё не уснул.
            crate::sched::block_on_irq_until(source, SLEEP_SLICE_MS, || self.has_used());
        }
        if self.take_used().is_some() {
            return Ok(());
        }
