...
//!
//!   0x0000_0080_0000_0000 .. окно пользовательской программы
//!       В таблицах ядра его нет: оно существует только в адресном
//!       пространстве самой программы, которое строится на время её работы
//!       копией корня ядра (см. `crate::user::space`). Адрес выбран так,
//!       чтобы всё окно поместилось в одну запись корневой таблицы, — тогда
//!       разбор пространства это обход одного поддерева.
//!
...
/// Размер обычной страницы, и единица всякого учёта памяти в ядре.
///
/// Раньше здесь стояло «ядро сознательно работает только со страницами 4 КиБ:
/// большие страницы усложняют защиту W^X». Довод был верен и остаётся верен —
/// но ровно для того, к чему относился. Сегменты ELF выровнены на 4 КиБ, и на
/// двухмегабайтной странице код неизбежно лёг бы в одну страницу с данными:
/// либо данные стали бы исполняемыми, либо код — записываемым.
///
/// Крупные страницы (фаза 42, [`HUGE_PAGE_SIZE`]) этого не касаются, потому что
/// живут в единственном месте, где выбирать нечего: безымянная память по
/// запросу. Она `READ | WRITE | USER` и никогда не исполняемая — смешивать в
/// ней нечего с нечем, и W^X ослабнуть не от чего. Образ программы, ядро и
/// отображённые файлы по-прежнему раскладываются страницами 4 КиБ, и довод
/// выше — причина, по которой так и останется.
pub const PAGE_SIZE: usize = 4096;
pub const PAGE_SHIFT: usize = 12;

/// Размер крупной страницы: 2 МиБ.
///
/// Ровно то, что покрывает одна запись предпоследнего уровня — 512 обычных
/// страниц, — и потому число здесь не выбрано, а посчитано: девять бит индекса
/// сверх двенадцати бит смещения. Одинаково на обеих архитектурах, потому что
/// одинаков шаг таблиц.
pub const HUGE_PAGE_SIZE: usize = 1 << (PAGE_SHIFT + 9);

/// Сколько обычных кадров в одной крупной странице.
pub const HUGE_PAGE_FRAMES: usize = HUGE_PAGE_SIZE / PAGE_SIZE;
...
/// Начало окна буферов DMA.
///
/// Отдельное окно, а не кусок кучи, потому что этим страницам нужен другой
/// атрибут памяти (см. [`PageFlags::DMA`]), а атрибут задаётся отображением. В
/// куче страницы уже отображены как обычная память, и переотобразить их иначе
/// значило бы менять свойства чужих аллокаций.
///
/// Лежит между кучей и стеком, не перекрывая ни то, ни другое: прямое
/// отображение заканчивается на [`HEAP_BASE`], куча — на `HEAP_BASE + HEAP_SIZE`.
pub const DMA_BASE: usize = 0xFFFF_D000_0000_0000;

/// Размер окна DMA.
///
/// 2 МиБ с большим запасом: кольца xHCI — это 4 КиБ каждое, контекст устройства
/// — 2 КиБ, буферы отчётов HID — 8 байт. Окно ограничено сознательно: аллокатор
/// внутри него не умеет освобождать (см. [`dma`]), и без предела утечка была бы
/// не видна до исчерпания памяти.
pub const DMA_SIZE: usize = 2 * 1024 * 1024;
...
impl PageFlags {
    ...
    /// Доступно из пользовательского режима (ring 3, EL0).
    ///
    /// Ставится только на страницы программы и только в её собственном
    /// адресном пространстве (см. `crate::user::space`). Ни одна страница
    /// ядра этого флага не несёт — и проверяется это не рассуждением, а
    /// обходом таблиц при каждом запуске программы.
    ...
    /// Обычная память, разделяемая с устройством, которое читает и пишет её
    /// напрямую (DMA): кольца дескрипторов и буферы данных.
    ///
    /// Не то же самое, что [`Self::DEVICE`], и разница принципиальная. Device-
    /// память на AArch64 — это `Device-nGnRnE`, где невыровненное обращение даёт
    /// отказ, а компилятор не вправе объединять доступы; структуру с полями
    /// разной ширины через такое отображение читать нельзя (этим уже пришлось
    /// заниматься в [`crate::vfs::ramdisk`]). Нужное свойство здесь другое —
    /// чтобы записанное процессором увидело устройство, а записанное
    /// устройством увидел процессор, то есть отсутствие кеша между ними.
    ///
    /// Что именно выставит арх-слой, от архитектуры и зависит: на x86-64 —
    /// ничего, DMA там когерентен по устройству шины; на AArch64 —
    /// `Normal Non-Cacheable`, потому что когерентности без IOMMU и без
    /// `dma-coherent` в описании устройства не обещано никем.
    pub const DMA: Self = Self(1 << 5);
    /// Память, в которую только **пишут** и притом помногу: кадровый буфер.
    ///
    /// Отличается от [`Self::DEVICE`] одним свойством, и оно решающее:
    /// процессору разрешено копить записи в своём буфере и отдавать их шине
    /// пачками (write combining). Для регистров устройства это недопустимо —
    /// там важен и порядок, и сам момент записи; для экрана недопустимо
    /// обратное.
    ///
    /// # Чего это стоило
    ///
    /// Кадровый буфер отображался как память устройства, то есть полностью
    /// некешируемой. В эмуляторе разницы нет никакой — там за «видеопамятью»
    /// стоит обычная память хоста. На живом ноутбуке каждая запись четырёх байт
    /// уходила в шину отдельной посылкой, а кадр — это четыре мегабайта:
    /// перетаскивание окна отставало от мыши на глаз. Это ровно тот класс
    /// дефектов, который стенд не ловит по устройству, а не по недосмотру.
