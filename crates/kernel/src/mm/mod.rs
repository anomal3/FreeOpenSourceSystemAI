...
/// Размер кучи.
///
/// Шестнадцати мегабайт хватало ровно до телефона, и это видно по счёту. Окно
/// на экране 720×1600 занимает его целиком: 664×1300 точек по четыре байта —
/// **три с половиной мегабайта одним куском**. Открыть поверх терминала
/// «Параметры» — уже семь; добавить полосу кадра, поверхности панели, меню и
/// шторки — и куча кончается. На аппарате это выглядело как «всё лагает и
/// рисуется сверху вниз», а в журнале стояло прямо: `KERNEL HEAP EXHAUSTED`,
/// свободно 5922 КиБ, и ни одного цельного блока нужного размера.
///
/// Шестьдесят четыре — это четыре окна во весь экран телефона плюс всё
/// остальное, и при этом капля для машины: у Redmi свободно почти два
/// гигабайта, у ноутбука — четыре. Цена — несколько таблиц страниц, то есть
/// десятки килобайт.
///
/// Почему не считать от экрана: размер кучи нужен раньше, чем ядро узнаёт про
/// экран (куча поднимается до разбора `BootInfo`), а число, зависящее от
/// машины, означало бы, что на одной из них мы никогда не проверим тот путь,
/// где памяти мало.
