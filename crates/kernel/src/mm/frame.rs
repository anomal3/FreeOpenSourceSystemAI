...
pub struct BitmapFrameAllocator {
    ...
    cursor: usize,
    /// Кадр ниже 640 КиБ, отложенный под код запуска остальных процессоров.
    ///
    /// Откладывается при построении карты, а не просится потом, и это не
    /// предусмотрительность: поиск идёт от нулевого кадра, и первыми уходят
    /// именно младшие — под таблицы страниц и кучу. К тому моменту, когда ядро
    /// дойдёт до второго процессора, ниже мегабайта не останется ничего.
    ///
    /// Ниже мегабайта — требование x86-64, а не выбор: проснувшийся процессор
    /// исполняется в реальном режиме, и адрес его первой инструкции задаётся
    /// номером страницы в восьмибитном поле `SIPI`. На AArch64 оперативной
    /// памяти так низко нет вовсе, и поле остаётся пустым.
    low_frame: Option<PhysAddr>,
}

/// Верхняя граница кадра под код запуска: 640 КиБ.
///
/// Мегабайт — предел адресации реального режима, но всё от `0xA0000` и выше —
/// видеопамять и ПЗУ старого PC, и прошивка описывает их не как память. Граница
/// здесь только страхует от карты, которая сказала бы иначе.
const LOW_FRAME_LIMIT: usize = 0xA0000 >> PAGE_SHIFT;
...
impl BitmapFrameAllocator {
    ...
    pub unsafe fn new(info: &BootInfo) -> Result<Self, FrameInitError> {
        ...
        // Кадр под код запуска остальных процессоров — самый старший свободный
        // ниже 640 КиБ, пока его не забрали таблицы страниц (см. `low_frame`).
        // Старший, а не первый: в самом низу реальный режим держит таблицу
        // векторов и область данных BIOS, и прошивка вправе описать их как
        // свободную память.
        let low_limit = LOW_FRAME_LIMIT.min(this.frames);
        if let Some(index) = (1..low_limit).rev().find(|&index| !this.is_used(index)) {
            this.reserve(index);
            this.low_frame = Some(frame_addr(index));
        }

        Ok(this)
    }

    /// Забрать кадр, отложенный под код запуска процессоров. Отдаётся один раз.
    pub fn take_low_frame(&mut self) -> Option<PhysAddr> {
        self.low_frame.take()
    }
    ...
    /// Выделить `count` подряд идущих обнулённых кадров, начиная с адреса,
    /// кратного `align` кадрам.
    ///
    /// Нужно крупным страницам, и «кратно» здесь не пожелание, а требование
    /// железа: двухмегабайтная запись хранит адрес в старших битах, а младшие
    /// обязаны быть нулём. Кадр не по границе — это не «чуть хуже», это отказ
    /// страницы с разбором зарезервированного бита.
    ///
    /// Отдельно от [`Self::allocate_contiguous`] намеренно: у той есть свой
    /// потребитель (окно DMA), которому выравнивание не нужно, и менять ему
    /// поведение ради нового вызывающего значило бы чинить не то.
    ///
    /// `None` — обычный ответ, а не поломка: непрерывного выровненного куска
    /// может не быть на исправной машине с раздробленной памятью, и вызывающий
    /// обязан уметь отступить на обычные страницы.
    pub fn allocate_aligned(&mut self, count: usize, align: usize) -> Option<PhysAddr> {
        let first = self.find_run_aligned(count, align)?;
        for index in first..(first + count) {
            self.set_used(index);
        }
        self.free -= count;
    ...
    /// Найти `count` подряд идущих свободных кадров, начиная с индекса,
    /// кратного `align`.
    ///
    /// Отдельный поиск, а не фильтр поверх [`Self::find_run`], и это не
    /// дублирование: тот возвращает **первый попавшийся** пробег и по
    /// построению почти никогда не попадает на границу — курсор после каждой
    /// выдачи стоит там, где кончилась прошлая. Проверять его ответ на
    /// кратность значило бы отвергать один подходящий кусок за другим, ни разу
    /// не поискав там, где надо.
    ///
    /// Зато выравненный поиск **дешевле** обычного, а не дороже: кандидатов в
    /// начало пробега в `align` раз меньше, и пробег, наткнувшийся на занятый
    /// кадр, продолжать бессмысленно — следующая проба начинается с очередной
    /// границы, а не со следующего кадра.
    fn find_run_aligned(&self, count: usize, align: usize) -> Option<usize> {
        if count == 0 || align == 0 || count > self.free {
            return None;
        }

        // Первая граница не раньше начала карты. Курсор здесь не участвует
        // намеренно: он оптимизирует повторные мелкие выдачи, а крупные идут
        // редко и им важнее найти хоть один кусок, чем найти близкий.
        ...
        while start + count <= self.frames {
            ...
            let mut clean = true;
            while index < end {
                // Байт из одних единиц — восемь занятых кадров подряд.
                if index & 7 == 0 && index + 8 <= end && self.byte(index >> 3) == 0xFF {
                    // Восемь занятых сразу: следующая проба обязана начаться за
                    // ними всеми, а не за первым из них.
                    index += 7;
                    clean = false;
                    break;
                }
            ...
            // Занятый кадр найден — следующая проба начинается с ближайшей
            // границы **за ним**: любая более ранняя накрыла бы тот же кадр.
            let next = index + 1;
...
pub fn with<R>(f: impl FnOnce(&mut BitmapFrameAllocator) -> R) -> Option<R> {
    // `try_lock`, а не `lock`, и это не оптимизация. Лок, который держит этот же
    // процессор, означает ровно одно: мы уже внутри `with` и пришли сюда из `f`.
    // Обычный `lock` в этом случае стал бы ждать сам себя — не паника, а вечное
    // молчание. Лок соседнего процессора `try_lock` дожидается, как и должен:
    // с фазы 43 сам лок помнит номер процессора, который его держит, и отличает
    // рекурсию от конкуренции (то, что стояло здесь пометкой `TODO(SMP)`).
...
/// Забрать кадр ниже 640 КиБ, отложенный под код запуска остальных процессоров.
///
/// `None` — такого кадра не нашлось при запуске или его уже забрали: отдаётся он
/// один раз.
pub fn take_low_frame() -> Option<PhysAddr> {
    with(BitmapFrameAllocator::take_low_frame).flatten()
}
