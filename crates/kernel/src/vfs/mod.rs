...
pub trait FileSystem: Send + Sync {
    ...
    ///
    /// Метод файловой системы, а не узла, и это не мелочь. Переименование
    /// затрагивает **два** каталога сразу, а у [`Node`] нет способа сослаться на
    /// чужой узел так, чтобы реализация могла с ним что-то сделать: `&dyn Node`
    /// пришлось бы приводить обратно к своему типу, то есть заводить в VFS
    /// механизм, существующий ради одной операции. Файловая система же знает обе
    /// стороны сама — оба пути лежат на ней.
    ///
    /// Права на каталоги спрашивает вызывающий ([`crate::fs::rename_as`]): здесь
    /// личности сеанса нет, как её нет и у остальных методов этого трейта.
