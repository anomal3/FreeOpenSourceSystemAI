...
/// Приглашение к вводу — ярко-зелёным, как в макете телефона (экран 06).
///
/// Цвет охватывает и пробел за `>`, и это не небрежность: стенд ищет в линии
/// строку `freeos> ` целиком, и сброс цвета между `>` и пробелом разорвал бы
/// её для каждого сценария разом. Цвет отличает приглашение от вывода команд —
/// на экране телефона без него всё сливается в один столбец.
const PROMPT: &str = "\x1b[92mfreeos> \x1b[0m";
...
/// пишет мимо него, из ядра, и с вытеснением врезается в середину строки:
///
/// ```text
///   volume      :   user        : #10 '/bin/sshd' as uid 0 ...
/// / flushed and marked clean
/// ```
///
/// Четыре сценария из восьмидесяти девяти упали именно так — не потому, что
/// система сделала не то, а потому, что о сделанном не удалось прочесть.
///
/// Лечится это тем, что строка собирается целиком ([`Line`]) и уходит одним
/// вызовом: в serial — под его `SpinLock`, при закрытых прерываниях, в окно —
/// одним `ui::write`. Разорвать такой вызов вытеснение не может.
///
/// # Чего здесь больше нет, и почему
///
/// До 25.09.2026 на время строки задача ещё и просила не отбирать у неё
/// процессор. Просьба пришла раньше [`Line`], когда строка уходила кусками, и
/// стояла на допущении «строка печатается миллисекунды». Оно перестало быть
/// верным, когда печать в оболочку стала рисовать окно терминала: замер
/// (`kernel::latency`) показал **961 мс** просьбы в отладочной сборке на
/// x86-64. Счётчик просьб был один на машину, так что всё это время не
/// вытеснялся никто, — и задача драйвера USB, разбуженная отпусканием
/// клавиши, ждала процессора до 448 мс. Автоповтор честно считал клавишу
/// зажатой: это и был открытый с 21.09 дефект «задача USB готова, но не
/// получает процессора».
...
///
/// Перевод строки — часть **того же** вызова, а не отдельный: два вызова подряд
/// это две разные атомарные записи, и чужая строка, попавшая между ними,
/// приклеивается к нашей. Ровно тот же приём, что у `kprintln!`.
...
pub fn task() {
    ...
    // идут в одну консоль, и перемешивать их незачем.
    //
    // Ждём **их**, а не «пока не останусь одна». Прежнее условие звучало как
    // «жива только эта задача» и было верно ровно до тех пор, пока обычными
    // задачами были лишь приглашение да демонстрация. На телефоне обычной стала
    // ещё и задача USB — намеренно, чтобы планировщик не гасил машину, пока
    // кабель остаётся единственным входом, — и условие перестало выполняться
    // когда-либо. Приглашение не выходило из ожидания, а значит не разбирало ни
    // одного события указателя и не обновляло часы: стол стоял нарисованным и
    // мёртвым, хотя тачскрин исправно слал координаты.
    while sched::demo_running() > 0 {
        ...
        while let Some(event) = input::next_event() {
            ...
            match editor.handle(event, &mut Out) {
                Edit::Submitted => {
                    ...
                    // И ещё раз — **после** отложенных команд, по той же
                    // причине, по которой отсчёт сдвинут выше. Отложенная
                    // команда — такая же команда: `sysupdate get` качает образ
                    // двадцать минут, и если он приехал не через редактор, а из
                    // остатка терминала, то к его концу отсчёт простоя старше
                    // предела — сеанс кончался в тот самый миг, когда оболочка
                    // печатала приглашение. Со стороны это выглядело как
                    // «система не приняла следующую команду»: стенд отправлял
                    // её через сто семьдесят восемь миллисекунд и не получал ни
                    // эха, ни ответа.
                    //
                    // Почему это видно только на x86-64: туда команда и попадает
                    // в остаток терминала. Уборка за программой печатает четыре
                    // строки, каждая перерисовывает окно 1280×800 в отладочной
                    // сборке, и следующая команда успевает прийти раньше, чем
                    // оболочка вернёт себе ввод. На ARM окно меньше, уборка
                    // быстрее, и та же команда идёт обычным путём.
                    idle_since = time::uptime_ms();
                }
...
fn run_command(line: &str) -> bool {
    ...
    match command {
        ...
        "input" => {
            ...
            // Третья строка — про то, что до ядра **не** дошло. Приёмник UART
            // теряет байт, когда тот приходит в полное FIFO, и потерянный байт
            // ввода снаружи выглядит как «система не приняла команду»: эхо
            // обрывается на середине, ответа нет. Без этого счётчика такое
