...
///
/// `true` с 25.09.2026. Ниже — история того, почему он почти две недели стоял
/// выключенным: полный прогон находил дефект, который подкачка вскрывала, а не
/// порождала.
///
...
///
/// # Пробовали включить 20.09.2026, вернули выключенным — и вот почему
///
/// Зацепка, названная абзацем выше, к тому дню была закрыта: **`is_running()`
/// больше не берёт спящий лок**. С фазы 55a признак «мы в третьем кольце»
/// лежит атомиком по слоту задачи, и обработчик отказа читает его, ничего не
/// ожидая. Роман 19.09 выбрал путь (1) — включить и доказать полным прогоном.
///
/// Прогон вышел красным на тридцати двух сценариях из 308 — **но ни один из
/// разобранных не оказался про подкачку**: сроки сна, считавшиеся в тиках,
/// автоповтор клавиши, оживший вместе с часами, и два устаревших числа. Пока
/// причины не разведены до конца, флаг стоит выключенным: включать его имеет
/// смысл на прогоне, где всё остальное уже зелёное, иначе вердикт будет о
/// смеси причин, а не о подкачке.
///
/// Что при этом проверено и работает: `paging-race` зелёный, подкачка
/// настоящая (у большой программы 1560 чтений вместо чтения наперёд).
///
/// # Прогон 21.09.2026 — ДЕФЕКТ ЖИВ, и вот его точное место
///
/// Второй полный прогон с флагом делался уже начисто: все тридцать два красных
/// прогона 20.09 были разобраны и починены, каждый сценарий перепроверен
/// поодиночке. То есть этот прогон отвечал про подкачку, а не про смесь.
///
/// Ответ: `filemap` на AArch64, программа `/bin/big`, 187 подкачанных страниц —
///
/// ```text
/// user : killed by an instruction abort at 0x5a3c27f8 (address 0x5a3c27f8)
/// ```
///
/// **Новое и главное:** адрес символизирован — это `aarch64_trap_common + 0x68`,
/// то есть **код самого ядра**, участок восстановления кадра ловушки. Прежняя
/// запись гласила «адрес один и тот же при разном смещении ядра, значит это не
/// код ядра, а вершина чьего-то стека минус восемь». Это оказалось неверно:
/// смещение ядра в том прогоне просто совпало. Задача, уснувшая в обработчике
/// отказа, после пробуждения делает `eret` по адресу **внутри заглушки
/// ловушки** — то есть ей достаётся чужой (или испорченный) `ELR_EL1`.
///
/// Где искать: вложенная ловушка поверх незавершённого кадра. Сон внутри
/// обработчика отказа безопасен сам по себе (доказано `paging-race` 18.09), но
/// не тогда, когда поверх него приходит вторая ловушка: кадр на стеке задачи
/// один, а записывают в него двое.
///
/// # Разобрано 25.09.2026 — и дело было не в сне
///
/// `+ 0x68` — это инструкция сразу за `msr elr_el1` в эпилоге заглушки.
/// Обработчик отказа из EL0 открывает прерывания (чтение засыпает) и
/// возвращался в эпилог, так их и не закрыв. Прерывание, пришедшее между
/// записью ELR и `eret`, при входе записывало в ELR_EL1 свою точку возврата —
/// адрес этой же инструкции в EL1, — а эпилог его уже не перечитывал. Отсюда
/// же и прежний адрес «при разном смещении один и тот же»: окно всегда одно.
/// Сон тут ни при чём, он только делал окно частым: подкачка — это тысячи
/// возвратов из отказа с открытыми прерываниями против единиц без неё. Эпилог
/// теперь закрывает прерывания до записи ELR/SPSR; то же окно было во входе
/// в программу (`aarch64_enter_user`) и закрыто там же. На x86-64 такого окна
/// нет: адрес возврата лежит в кадре `iretq` на стеке, а не в регистре.
const IMAGE_PAGED_ON_DEMAND: bool = true;
