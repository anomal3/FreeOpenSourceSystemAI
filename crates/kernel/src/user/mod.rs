...
///
/// Хранится в [`Program`] по возрастанию адреса. Нужна двум вещам: найти, куда
/// положить следующую, и узнать в `SYS_MUNMAP`, что именно просят снять.
///
...
struct Mapping {
    ...
    base: usize,
    /// Длина в страницах. Ноль означает пустую запись таблицы.
    pages: usize,
}
...
pub enum MmapError {
    /// Просьба бессмысленна: нулевая длина, или снять просят не то, что выдавали.
    BadRequest,
...
pub struct Program {
    ...
    ///
    /// Живут первые [`Program::mapped`] записей; остальные — нули. Массив, а не
    /// список: областей самое большее [`MAX_MAPPINGS`], а куча в этом месте
    /// означала бы выделение памяти внутри выделения памяти.
    mappings: [Mapping; MAX_MAPPINGS],
    /// Сколько записей [`Program::mappings`] заняты.
    mapped: usize,
}
...
impl Program {
    ...
    ///
    /// Память выделяется **сразу и целиком**: двести пятьдесят шесть мегабайт
    /// нулей — это работа, и её видно, но зато выданный адрес с этой секунды
    /// действителен весь, а не до первого обращения. Ленивая выдача требует
    /// обработчика отказа страницы, которого у системы пока нет: отказ из
    /// третьего кольца сейчас снимает программу, и превращать его в рабочий
    /// механизм — фаза 41, вместе с отображением файлов, ради которого он и
    /// нужен по-настоящему.
    ///
    ...
    fn mmap(&mut self, len: usize) -> Result<usize, MmapError> {
        ...
        if self.mapped == MAX_MAPPINGS {
            return Err(MmapError::Limit);
        }

        let base = self.find_gap(bytes).ok_or(MmapError::Limit)?;

        // SAFETY: `find_gap` вернул адрес внутри области запроса, не пересекающийся
        // ни с одной уже выданной; образ и стек лежат ниже [`MMAP_BASE`].
        unsafe {
            self.space.map_anon(
                VirtAddr::new(base),
                pages,
        ...
        self.remember(Mapping { base, pages });
        Ok(base)
    }

    /// Вернуть системе область, выданную [`Program::mmap`].
    ///
    ...
    fn munmap(&mut self, addr: usize, len: usize) -> Result<(), MmapError> {
        ...
        let pages = len.div_ceil(PAGE_SIZE);
        let index = (0..self.mapped)
            .find(|&i| self.mappings[i].base == addr && self.mappings[i].pages == pages)
        ...
        let freed = unsafe {
            self.space.unmap_range(VirtAddr::new(addr), pages)
        };
        if freed != pages {
            // Дыра в области, которую никто не мог проделать. Молчать об этом
            // нельзя: она означает, что таблицы разошлись с этой записью, а
            // значит счёт кадров больше ничего не доказывает.
            kprintln!(
                "  user        : WARNING: mmap region at {addr:#x} gave back {freed} pages of {pages}"
            );
        }

        // Запись убирается сдвигом, чтобы таблица осталась упорядоченной по
        // адресу: на ней стоит поиск свободного места.
        self.mappings.copy_within(index + 1..self.mapped, index);
        self.mapped -= 1;
        self.mappings[self.mapped] = Mapping { base: 0, pages: 0 };
        Ok(())
    }
    ...
    fn find_gap(&self, bytes: usize) -> Option<usize> {
        ...
        for region in self.mappings.iter().take(self.mapped) {
    ...
    fn remember(&mut self, region: Mapping) {
        let at = (0..self.mapped)
            .find(|&i| self.mappings[i].base > region.base)
            .unwrap_or(self.mapped);
        self.mappings.copy_within(at..self.mapped, at + 1);
        self.mappings[at] = region;
        self.mapped += 1;
    }
}
...
fn run(
    ...
    let slot = sched::current_slot();
    {
        ...
        *entry = Some(Program {
            ...
            stdin,
            stdout,
            mappings: [Mapping { base: 0, pages: 0 }; MAX_MAPPINGS],
            mapped: 0,
        });
    }
