...
impl Reader {
    ...
    /// Закрыт ли другой конец: писателей больше нет.
    ///
    /// Отличается от [`Reader::ready`] тем, что не считает готовностью
    /// непрочитанные байты: спрашивает это `SYS_POLL`, которому надо сказать
    /// «труба кончилась» отдельно от «в трубе что-то есть». Оба утверждения
    /// бывают истинны разом — писатель ушёл, оставив данные, — и программа,
    /// которой сообщили только второе, дочитает их и снова уснёт.
    #[must_use]
    pub fn hangup(&self) -> bool {
        self.pipe.inner.lock().writers == 0
    }
}
...
impl Writer {
    ...
    /// Есть ли куда писать прямо сейчас — или писать уже некому.
    ///
    /// Один ответ на оба случая, как и у [`Reader::ready`], и по той же
    /// причине: спрашивает это тот, кто решает, засыпать ли, а засыпать нельзя
    /// ни при свободном месте, ни при исчезнувшем читателе. Что именно из двух
    /// случилось, спрашивающий узнаёт попыткой записи — она отвечает
    /// [`PipeError::Broken`].
    #[must_use]
    pub fn ready(&self) -> bool {
        let inner = self.pipe.inner.lock();
    ...
    /// Не осталось ли читателей: писать некому.
    #[must_use]
    pub fn hangup(&self) -> bool {
        self.pipe.inner.lock().readers == 0
    }
}
