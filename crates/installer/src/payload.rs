...
///
/// Длина массива написана числом, и это ловушка, которая уже сработала: в фазе
/// 37 её увеличили до 22, а строки для `echoc` и `sshd` не дописали. Установщик
...
/// Ловушка сработала второй раз, и на этот раз молча.
///
/// В фазах 47b и 47c монитор системы и файловый менеджер уехали из ядра в
/// программы (`sysmon`, `files`), а сюда их дописать забыли — вместе с `posix`
/// и `winshow`. Установщик при этом **собрался**: длина массива совпадала с его
/// содержимым, а с `USER_PROGRAMS` её никто не сверял. В итоге установленная
/// система оставалась без файлового менеджера и без монитора: значок «Файлы» на
/// столе показывал бы окно, которое не открывается.
///
/// Нашлось это в фазе 48a — сценарием, который просто перечислил `/bin`. Чтобы
...
const PROGRAMS: [(&CStr16, &str); 40] = [
    ...
    (cstr16!("\\FREEOS\\BIN\\NAP"), "nap"),
    (cstr16!("\\FREEOS\\BIN\\SAVE"), "save"),
    (cstr16!("\\FREEOS\\BIN\\WC"), "wc"),
    (cstr16!("\\FREEOS\\BIN\\LS"), "ls"),
    ...
    // Программа, переполняющая свой стек нарочно: канарейка стека (очередь
    // второго разбора, пункт 1) проверяется и на установленной системе.
    (cstr16!("\\FREEOS\\BIN\\SMASH"), "smash"),
    ...
    // смысла фазы — чужой язык не должен требовать особого обращения.
    (cstr16!("\\FREEOS\\BIN\\CDEMO"), "cdemo"),
    // Программы с окнами — те, ради которых были фазы 47a-47c. Без них
