...
///
/// Это имя стоит **внутри** запроса на вход (RFC 4252 §5): «впустите меня, я
/// хочу вот к этой службе».
pub const SERVICE: &str = "ssh-connection";

/// Имя службы, которую клиент просит **до** входа (RFC 4253 §10).
///
/// Отдельная константа, и это не педантизм: имён два, они разные, и путать их
/// нельзя. Клиент сначала просит `ssh-userauth` — «дайте поговорить с тем, кто
/// проверяет ключи», — и только потом внутри самой проверки называет
/// [`SERVICE`], службу, к которой хочет попасть после входа.
///
/// Одна константа на оба случая уже стоила фазы: сервер сверял имя из
/// `SERVICE_REQUEST` с `ssh-connection`, отвечал «нет такой службы» и разрывал
/// соединение **до** первой попытки входа. Со стороны клиента это выглядит как
/// «сервер не хочет разговаривать», и ни один ключ до сервера не доезжал.
