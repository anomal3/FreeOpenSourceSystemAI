...
pub mod font;
pub mod glyphicon;
/// Общий набор элементов программ с окнами: панели, вкладки, таблица, меню
/// (фаза С8).
pub mod kit;
/// Словарь элементов рабочего стола: строка списка, кнопка, плитка значка.
///
/// Переехал сюда из ядра в фазе 47c вместе с [`theme`]. Причина в том, что
/// окна перестали быть частью ядра: файловый менеджер рисует список,
/// а монитор — свои строки, и оба обязаны выглядеть так же, как рисует
/// стол. Оставить словарь в ядре значило бы либо второй словарь у программ,
/// либо интерфейс, собранный из двух разных.
pub mod paint;
pub mod text;
/// Палитра и размеры. См. [`paint`] — переехала оттуда же и по той же причине.
pub mod theme;
pub mod typeface;
pub mod widget;
...
/// Старший байт точки: непрозрачно. См. [`Color::pixel`].
...
/// Формат «красный в младшем байте» — значение для [`use_raw_format`].
pub const PIXEL_RGB: u32 = FORMAT_RGB;
/// Формат «синий в младшем байте».
pub const PIXEL_BGR: u32 = FORMAT_BGR;

/// Формат пикселя этой машины числом. Ноль — ещё не известен.
///
/// Нужен ядру, чтобы сказать его программе: окно с фазы 47a есть и у неё, а
/// [`Screen`] — только у ядра.
#[must_use]
pub fn format_code() -> u32 {
    FORMAT.load(Ordering::Acquire)
}

/// Принять формат пикселя числом — так его узнаёт программа.
///
/// Программа рисует в поверхность своего окна, но [`Screen`] не создаёт: экран
/// принадлежит ядру, и приватный путь через [`Screen::new`] ей недоступен. Без
/// этого вызова [`Color::pixel`] отвечала бы нулём, и окно программы вышло бы
/// сплошь чёрным — при совершенно исправной отрисовке, что и делает такую
/// ошибку особенно дорогой в поисках.
///
/// Значение берётся из ответа ядра, а не выдумывается. Всё, что не [`PIXEL_RGB`]
/// и не [`PIXEL_BGR`], означает «формат неизвестен»: чёрное окно честнее, чем
/// нарисованное наугад, где красное перепутано с синим.
pub fn use_raw_format(code: u32) {
    let known = match code {
        FORMAT_RGB | FORMAT_BGR => code,
        _ => FORMAT_UNSET,
    };
    FORMAT.store(known, Ordering::Release);
}
...
impl Color {
    ...
    ///
    /// Старший байт — прозрачность, и он всегда единицы. В описании UEFI он
    /// назван зарезервированным и никем не читается; у телефона кадровый буфер
    /// — слой наложения, который контроллер смешивает с фоном, и ноль там
    /// означает чёрный экран при исправно идущей отрисовке.
    #[must_use]
    pub fn pixel(self) -> u32 {
        ...
        match FORMAT.load(Ordering::Acquire) {
            ...
            FORMAT_RGB => OPAQUE | r | (g << 8) | (b << 16),
            FORMAT_BGR => OPAQUE | b | (g << 8) | (r << 16),
            _ => 0,
        }
    }
