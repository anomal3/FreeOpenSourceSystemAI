...
//!    А та, у которой символа нет, — именем из того же договора, и печатается
//!    оно словом. Стрелки понадобились списку файлового менеджера (фаза 47c):
//!    до неё клавиша без символа события не давала вовсе, и это делало
//!    невозможной любую программу, которую листают.
//!
...
use user_progs::{
    Line, WIN_CLOSE, WIN_KEY, WIN_POINTER, Window, exit, key_name, monotonic_ms, munmap, nanosleep,
    println,
};
        ...
        while let Some(event) = window.next_event() {
            ...
            match event.kind {
                WIN_KEY => {
                    ...
                    match key_name(event.code) {
                        // Клавиша без символа приезжает именем из договора, а
                        // не числом. Печатается тоже именем: число `16777219`
                        // не говорит ничего ни человеку, ни стенду, а сверять
                        // стенд с числом значило бы вписать в него границу
                        // диапазона второй раз.
                        Some(name) => line.str("named ").str(name),
                        // Символ печатается как число: собрать из него строку
                        // нечем — разбора UTF-8 в обвязке нет, а врать про
                        // букву, которой не видел, программа не станет.
                        None => line.num(u64::from(event.code)),
                    };
                    line.end();
