...
///
/// Двести пятьдесят шесть мегабайт — половина того, что задаче полагается по
/// запросу (`MMAP_MAX_BYTES` в ядре): сборка читается в кучу целиком, и
/// вторая половина остаётся самой программе. Было четыре мегабайта — с запасом
/// от нашей CoreLib в полмегабайта; Роман: «программы могут быть большие».
