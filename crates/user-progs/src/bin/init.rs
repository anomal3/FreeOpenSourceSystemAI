...
fn reap(service: &mut Service, now: u64) -> bool {
    ...
    // говорит, что настраивать нечего, и выходит с нулём — а супервизор,
    // не различающий эти два случая, запускал бы его по кругу до конца работы
    // системы, засоряя журнал и сдвигая номера задач под ногами у всех
    // остальных. Ровно то же различие в чужих системах называется
