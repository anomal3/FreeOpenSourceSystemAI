...
extern "C" fn worker(index: usize) -> ! {
    ...
    for _ in 0..STEPS {
        ...
        // Уступка процессора ПОД ЗАМКОМ — то, что делает встречу потоков
        // неизбежной, а не вероятной.
        //
        // Первая редакция жгла здесь четыреста пустых витков и полагалась на
        // то, что квант истечёт посреди участка. В отладочной сборке на одном
        // процессоре так и выходило, а в release и на ARM — нет: «усыпил 0
        // раз» при сошедшемся счётчике, то есть зелёная проверка, не
        // проверившая ничего. Уступка отдаёт процессор соседу гарантированно,
        // и соседу остаётся только уснуть на замке.
        user_progs::yield_now();
        LOCK.release();
    }
