...
//!
//! # Что здесь оказалось, кроме настроек
//!
//! Модуль [`winpath`] — перевод путей, написанных по-виндовому, и подписи
//! каталогов. Повод у него ровно тот же, по которому крейт существует: это
//! чистая работа со строкой, ошибка в ней стоит загрузки системы, чтобы её
//! заметить, а зовут её **с обеих сторон границы** — из ядерной оболочки и из
//! программ третьего кольца. Две реализации одного перевода разошлись бы, и
//! разошлись бы молча.
